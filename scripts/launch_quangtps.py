...
def launch_quangtps(with_3d=True, debug=False):
    ...
    if quangtps_dir is None:
        print(
            "QuangTPS module not found. Please install it or run from the repository."
        )
        return False
    ...
    try:
        ...
        # Launch QuangTPS using the proper function
        ...
        print(f"Error launching QuangTPS: {e}")
        if debug:
            import traceback
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Launch QuangTPS Treatment Planning System"
    )
    parser.add_argument("--no-3d", action="store_true", help="Disable 3D visualization")
    parser.add_argument("--debug", action="store_true", help="Enable debug mode")
    parser.add_argument(
        "--skip-dependency-check", action="store_true", help="Skip dependency check"
    )
