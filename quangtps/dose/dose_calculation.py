...
class DoseRegionOfInterest:
    ...
    def calculate_bounds_from_structure(self, structures: Dict[str, np.ndarray], image_info: Dict[str, Any]) -> List[Tuple[float, float]]:
        """
    ...
    def get_bounds(self, structures: Dict[str, np.ndarray] = None, image_info: Dict[str, Any] = None) -> List[Tuple[float, float]]:
        """
...
class DoseCalculator:
    ...
    def _prepare_dose_grid(self, patient_ct: sitk.Image, roi: DoseRegionOfInterest, structures: Dict[str, np.ndarray]) -> DoseGrid:
        """
    ...
    def calculate_dose(self, 
        ...
        try:
            ...
            if self.parameters.get("parallel_beam_calculation", True) and len(beams) > 1:
                # Tính toán song song
                beam_results = self._calculate_beams_parallel(patient_ct, structures, beams, reference_grid)
            else:
                # Tính toán tuần tự
                beam_results = self._calculate_beams_sequential(patient_ct, structures, beams, reference_grid)
    ...
    def _calculate_beams_sequential(self, 
                                  patient_ct: sitk.Image, 
                                  structures: Dict[str, np.ndarray], 
                                  beams: List[Dict[str, Any]], 
                                  reference_grid: DoseGrid) -> List[DoseGrid]:
        """
        ...
        for i, beam in enumerate(beams):
            ...
            self._update_status(progress, f"Calculating beam {i+1}/{len(beams)}: {beam.get('name', f'Beam {i+1}')}")
    ...
    def _calculate_beams_parallel(self, 
                                patient_ct: sitk.Image, 
                                structures: Dict[str, np.ndarray], 
                                beams: List[Dict[str, Any]], 
                                reference_grid: DoseGrid) -> List[DoseGrid]:
