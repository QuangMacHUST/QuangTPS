...
class DoseColorwash:
    ...
    def __init__(self, color_scheme: Optional[ColorScheme] = None):
        ...
        self.normalization_value = (
            100.0  # Default normalization (100% = prescription dose)
        )
    ...
    def display_2d(
        ...
        anatomy_slice: Optional[np.ndarray] = None,
        axis: str = "axial",
        ...
        if anatomy_slice is not None and self.background_visible:
            ...
            if anatomy_max > anatomy_min:
                anatomy_norm = (anatomy_slice - anatomy_min) / (
                    anatomy_max - anatomy_min
                )
            else:
            ...
            ax.imshow(
                anatomy_norm, cmap="gray", interpolation="nearest", origin="lower"
            )
        ...
        if dose_max > 0:
            ...
            im = ax.imshow(
                dose_norm,
                cmap=self.color_scheme.dose_colormap,
                alpha=self.alpha,
                vmin=0,
                vmax=1.0,
    ...
    def display_isodose_lines(
        ...
        if dose_max > 0:
            dose_norm = (
                dose_slice / self.normalization_value * 100
            ...
            # Draw contours
            contours = ax.contour(
                dose_norm, levels=isodose_levels, colors=isodose_colors, linewidths=2
            )
    ...
    def display_3d_isodose_surfaces(
        self,
        dose_3d: np.ndarray,
        ...
        if dose_max > 0:
            ...
            legend_elements = [
                Patch(
                    facecolor=self.color_scheme.isodose_colors[i],
                    edgecolor="black",
...
class DoseProfiler:
    ...
    def extract_line_profile(
        ...
        for dim in range(3):
            if (
                start_point[dim] < 0
                or start_point[dim] >= dose_3d.shape[dim]
                or end_point[dim] < 0
                or end_point[dim] >= dose_3d.shape[dim]
            ):
                raise ValueError(
                    f"Points must be within dose grid bounds: {dose_3d.shape}"
                )
        ...
        for i in range(num_samples):
            z, y, x = (
                int(round(points_z[i])),
                int(round(points_y[i])),
                int(round(points_x[i])),
            )
            # Ensure bounds
...
class DVHPlotter:
    ...
    def plot_cumulative_dvh(
        ...
        if relative_dose and prescription_dose is None:
            raise ValueError(
                "Prescription dose must be provided for relative dose display"
            )
        ...
        for i, (struct_name, struct_mask) in enumerate(structures.items()):
            ...
            marker_idx = (i // (len(self.colors) * len(self.line_styles))) % len(
                self.markers
            )
    ...
    def plot_differential_dvh(
        ...
        for i, (struct_name, struct_mask) in enumerate(structures.items()):
            ...
            line = ax.fill_between(
                dose_bins, volume_bins, color=self.colors[color_idx], alpha=0.3
            )
