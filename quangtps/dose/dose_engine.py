...
class DoseCalculationAlgorithm(Enum):
    ...
    def from_string(algorithm_name: str) -> "DoseCalculationAlgorithm":
        ...
        for algo in DoseCalculationAlgorithm:
            if (
                algo.name.lower() == algorithm_name.lower()
                or algo.value.lower() == algorithm_name.lower()
            ):
                return algo
...
class DoseEngine:
    ...
    def _load_algorithm_implementations(self):
        ...
        try:
            ...
            try:
                ...
                self.algorithm_implementers[DoseCalculationAlgorithm.AAA] = (
                    AAAAlgorithm()
                )
            except ImportError:
            ...
            try:
                ...
                self.algorithm_implementers[DoseCalculationAlgorithm.ACUROS] = (
                    AcurosXBAlgorithm()
                )
            except ImportError:
            ...
            try:
                from quangtps.dose.algorithms.conv_superposition import (
                    ConvolutionSuperpositionAlgorithm,
                )

                self.algorithm_implementers[
                    DoseCalculationAlgorithm.CONV_SUPERPOSITION
                ] = ConvolutionSuperpositionAlgorithm()
            except ImportError:
            ...
            try:
                ...
                self.algorithm_implementers[DoseCalculationAlgorithm.GBBS] = (
                    GBBSAlgorithm()
                )
            except ImportError:
            ...
            logger.info(
                f"Loaded {len(self.algorithm_implementers)} dose calculation algorithm implementers"
            )
    ...
    def _initialize_algorithm(self):
        ...
        if self.algorithm in self.algorithm_implementers:
            ...
            logger.info(
                f"Initialized dose calculation algorithm: {self.algorithm.value}"
            )
        else:
            logger.warning(
                f"No implementation found for algorithm: {self.algorithm.value}"
            )
            self.algorithm_instance = None
    ...
    def calculate(
        ...
        if self.algorithm_instance is None:
            raise AlgorithmError(
                f"No implementation available for algorithm: {self.algorithm.value}"
            )
        ...
        try:
            ...
            for i, beam_data in enumerate(beams):
                logger.info(
                    f"Calculating beam {i + 1}/{len(beams)}: {beam_data.get('name', f'Beam {i + 1}')}"
                )
                ...
                if (
                    beam_dose
                    and hasattr(beam_dose, "dose")
                    and beam_dose.dose is not None
                ):
            ...
            if reference_grid is None:
                ...
                result_grid = DoseGrid.create_from_data(
                    dose_data=dose_grid, reference_image=patient_ct
                )
            else:
            ...
            raise AlgorithmError(
                f"Error calculating dose with {self.algorithm.value}: {str(e)}"
            )
    ...
    def export_dvh(
        self, structure_name: str, output_file: str, format: str = "csv"
    ) -> bool:
        """
...
class DoseCalculationImplementer(ABC):
    ...
    def export_dvh(
        ...
        try:
            ...
            logger.info(
                f"Exporting DVH for structure {structure_name} to {output_file}"
            )
            return True
        except Exception as e:
    ...
    def generate_report(
        self, dose_grid: DoseGrid, output_file: str, parameters: Dict[str, Any]
    ) -> bool:
        """
        Tạo báo cáo tính toán liều.
...
# Đăng ký các implementer cho các thuật toán tính liều
_DOSE_ALGORITHM_IMPLEMENTERS = {}
_DOSE_ALGORITHM_GPU_SUPPORT = {}  # Lưu trữ thông tin về hỗ trợ GPU


def register_dose_algorithm_implementer(
    algorithm: DoseCalculationAlgorithm,
    implementer: DoseCalculationImplementer,
    gpu_supported: bool = False,
) -> None:
    """
    Đăng ký một implementer cho một thuật toán tính liều.

    Parameters:
        algorithm: Thuật toán tính liều
        implementer: Đối tượng thực thi thuật toán
        gpu_supported: True nếu implementer hỗ trợ tính toán GPU
    """
    global _DOSE_ALGORITHM_IMPLEMENTERS, _DOSE_ALGORITHM_GPU_SUPPORT

    _DOSE_ALGORITHM_IMPLEMENTERS[algorithm] = implementer
    _DOSE_ALGORITHM_GPU_SUPPORT[algorithm] = gpu_supported

    logger.info(
        f"Đã đăng ký implementer cho thuật toán {algorithm}"
        + (" với hỗ trợ GPU" if gpu_supported else "")
    )


def get_dose_algorithm_implementer(
    algorithm: DoseCalculationAlgorithm,
) -> Optional[DoseCalculationImplementer]:
    """
    Lấy implementer cho một thuật toán tính liều.

    Parameters:
        algorithm: Thuật toán tính liều

    Returns:
        Đối tượng thực thi thuật toán hoặc None nếu không tìm thấy
    """
    return _DOSE_ALGORITHM_IMPLEMENTERS.get(algorithm)


def is_gpu_supported(algorithm: DoseCalculationAlgorithm) -> bool:
    """
    Kiểm tra xem thuật toán có hỗ trợ tính toán GPU không.

    Parameters:
        algorithm: Thuật toán tính liều

    Returns:
        True nếu thuật toán hỗ trợ GPU, False nếu không
    """
    return _DOSE_ALGORITHM_GPU_SUPPORT.get(algorithm, False)


def get_supported_dose_algorithms() -> List[DoseCalculationAlgorithm]:
    """
    Lấy danh sách các thuật toán tính liều được hỗ trợ.

    Returns:
        Danh sách các thuật toán tính liều
    """
    return list(_DOSE_ALGORITHM_IMPLEMENTERS.keys())


def get_gpu_supported_algorithms() -> List[DoseCalculationAlgorithm]:
    """
    Lấy danh sách các thuật toán tính liều hỗ trợ GPU.

    Returns:
        Danh sách các thuật toán tính liều hỗ trợ GPU
    """
    return [alg for alg, supported in _DOSE_ALGORITHM_GPU_SUPPORT.items() if supported]
...
def calculate_dose(
    ...
    """
    Tính toán phân bố liều sử dụng thuật toán đã chọn.
    ...
    Parameters:
        ...
        dose_grid: Lưới liều
        calculation_options: Các tùy chọn tính toán (tùy chọn)
        use_gpu: True để sử dụng GPU nếu có thể

    Returns:
        Mảng phân bố liều

    Raises:
        ValueError: Nếu thuật toán không được hỗ trợ
        AlgorithmError: Nếu có lỗi trong quá trình tính toán
    """
    # Kiểm tra xem thuật toán có được hỗ trợ không
    implementer = get_dose_algorithm_implementer(algorithm)
    if implementer is None:
        raise ValueError(f"Thuật toán {algorithm} không được hỗ trợ")

    # Kiểm tra xem có sử dụng GPU không
    if use_gpu and not is_gpu_supported(algorithm):
        logger.warning(
            f"Thuật toán {algorithm} không hỗ trợ GPU. Sử dụng CPU thay thế."
        )
        use_gpu = False

    # Đảm bảo calculation_options là một từ điển
    if calculation_options is None:
        calculation_options = {}

    # Thêm thông tin về sử dụng GPU
    calculation_options["use_gpu"] = use_gpu

    # Thực hiện tính toán
    try:
        return implementer.calculate(
            beam_data, patient_data, dose_grid, calculation_options
        )
    except Exception as e:
        logger.exception(f"Lỗi khi tính toán sử dụng {algorithm}")
        raise AlgorithmError(f"Lỗi tính toán liều với {algorithm}: {str(e)}")


# Đăng ký các thuật toán mặc định
def register_default_algorithms():
    """Đăng ký các thuật toán tính liều mặc định."""
    try:
        from quangtps.dose.algorithms.pencil_beam import PencilBeamImplementer

        register_dose_algorithm_implementer(
            DoseCalculationAlgorithm.PENCIL_BEAM, PencilBeamImplementer()
        )
    except ImportError:
        logger.warning("Không tìm thấy module PencilBeamImplementer")

    try:
        from quangtps.dose.algorithms.aaa import AAAImplementer

        register_dose_algorithm_implementer(
            DoseCalculationAlgorithm.AAA, AAAImplementer()
        )
    except ImportError:
        logger.warning("Không tìm thấy module AAAImplementer")

    # Đăng ký Acuros GPU nếu có, ưu tiên phiên bản GPU
    try:
        from quangtps.dose.algorithms.acuros_gpu import (
            register_algorithm as register_acuros_gpu,
        )

        register_acuros_gpu()
        logger.info("Đã đăng ký thuật toán Acuros XB với tăng tốc GPU")
    except ImportError:
        logger.warning("Không tìm thấy module Acuros XB GPU, thử sử dụng phiên bản CPU")
        try:
            # Chỉ sử dụng phiên bản CPU nếu không có GPU
            from quangtps.dose.algorithms.acuros import AcurosXBImplementer

            # Đảm bảo rằng ACUROS_XB được định nghĩa trong enum
            if hasattr(DoseCalculationAlgorithm, "ACUROS_XB"):
                implementer = AcurosXBImplementer()
                # Triển khai hai abstract methods còn thiếu
                if not hasattr(implementer, "get_description"):
                    implementer.get_description = (
                        lambda: "Acuros XB cho tính toán liều chính xác trong môi trường không đồng nhất."
                    )
                if not hasattr(implementer, "get_parameters_info"):
                    implementer.get_parameters_info = lambda: {}

                register_dose_algorithm_implementer(
                    DoseCalculationAlgorithm.ACUROS_XB, implementer
                )
                logger.info("Đã đăng ký thuật toán Acuros XB phiên bản CPU")
        except ImportError:
            logger.warning("Không tìm thấy module Acuros XB")
        except Exception as e:
            logger.error(f"Lỗi khi đăng ký Acuros XB: {str(e)}")

    # Đăng ký Monte Carlo nếu có
    try:
        from quangtps.dose.algorithms.monte_carlo import MonteCarloImplementer

        register_dose_algorithm_implementer(
            DoseCalculationAlgorithm.MONTE_CARLO,
            MonteCarloImplementer(),
            gpu_supported=True,
        )
        logger.info("Đã đăng ký thuật toán Monte Carlo")
    except ImportError:
        logger.warning("Không tìm thấy module Monte Carlo")


# Đăng ký các thuật toán mặc định khi import module
