...
from quangtps.core.types import DoseUnit, VolumeUnit
    ...
    def is_satisfied(self, value: float) -> bool:
        ...
        if self.direction == ConstraintDirection.UPPER:
            return (
                ...
                in [ConstraintType.MAX_DOSE, ConstraintType.MEAN_DOSE]
                else value <= self.volume_value
            )
        elif self.direction == ConstraintDirection.LOWER:
            return (
                value >= self.dose_value
                if self.constraint_type
                in [ConstraintType.MIN_DOSE, ConstraintType.MEAN_DOSE]
                else value >= self.volume_value
            )
            ...
            return (
                abs(
                    ...
                    - (
                        ...
                        if self.constraint_type
                        in [
                            ConstraintType.MAX_DOSE,
                            ConstraintType.MIN_DOSE,
                            ConstraintType.MEAN_DOSE,
                        ]
                        else self.volume_value
                    )
                )
                < 1e-6
            )
...
class DoseConstraints:
    ...
    def get_constraints_by_structure(
    ...
    def get_constraints_by_priority(
    ...
    def evaluate_constraints(
        self, evaluation_data: Dict[str, Dict[str, Any]]
    ) -> Dict[str, Any]:
        """
        ...
        for constraint in self.constraints:
            if not isinstance(
                constraint,
                (
                    OptimizationDoseVolumeConstraint,
                    MaxDoseConstraint,
                    MinDoseConstraint,
                    MeanDoseConstraint,
                ),
            ...
            if structure_name not in evaluation_data:
                results["warnings"].append(
                    f"Không tìm thấy dữ liệu cho cấu trúc {structure_name}"
                )
                continue
            ...
            if "dvh" in structure_data:
                ...
                dvh_analysis = {
                    ...
                    "gradient_metrics": {"R50": dvh.get_r50(), "R100": dvh.get_r100()},
                    "conformity_metrics": {
        ...
        if results["total_constraints"] > 0:
            results["percent_met"] = (
                results["constraints_met"] / results["total_constraints"]
            ) * 100
        ...
        try:
            ...
            try:
                ...
                if isinstance(plan, PlanEvaluation):
                    ...
                    if hasattr(plan, "get_metrics") and callable(
                        getattr(plan, "get_metrics")
                    ):
                        metrics_fn = getattr(plan, "get_metrics")
                        evaluation_data = metrics_fn()
                else:
                    # Xử lý khi plan không phải là PlanEvaluation
                    if hasattr(plan, "get_evaluation") and callable(
                        getattr(plan, "get_evaluation")
                    ):
                        # Lấy evaluation từ plan
                        get_eval_fn = getattr(plan, "get_evaluation")
                        evaluation = get_eval_fn()

                        if hasattr(evaluation, "get_metrics") and callable(
                            getattr(evaluation, "get_metrics")
                        ):
                        ...
                        try:
                            ...
                            logging.warning(
                                f"Could not create PlanEvaluation from plan: {e}"
                            )
            except ImportError:
                logging.warning(
                    "Could not import PlanEvaluation, skipping plan evaluation"
                )
    ...
    def _generate_html_report(self, report_data: Dict[str, Any]) -> str:
        ...
        if "constraints_by_priority" in report_data:
            ...
            for priority, count in report_data.get(
                "constraints_by_priority", {}
            ).items():
                html += f"""
                    <tr>
        ...
        if "constraints_by_type" in report_data:
            ...
            for constraint_type, count in report_data.get(
                "constraints_by_type", {}
            ).items():
                html += f"""
                    <tr>
    ...
    def _calculate_priority_score(
        self, results: Dict[str, Any], priority: str
    ) -> float:
