...
class RegistrationResult:
    ...
    def apply_transform_to_image(self, image: Image) -> Image:
        ...
        transformed_image = Image(
            ...
            pixel_array=transformed_array,
            spacing=image.spacing,
            origin=image.origin,
            direction=image.direction,
            description=f"Transformed from {image.id}"
        )
    ...
    def apply_transform_to_structure(self, structure: Structure) -> Structure:
        ...
        transformed_structure = Structure(
            ...
            description=f"Transformed from {structure.id}"
        )
        
        # Áp dụng phép biến đổi cho mỗi contour
        for contour in structure.contours:
            # Tạo đối tượng mesh hoặc điểm và áp dụng phép biến đổi
            # Trong thực tế, cần thực hiện chi tiết hơn
            transformed_structure.contours.append(contour)  # Tạm thời giữ nguyên
...
class ImageRegistration:
    ...
    def _get_sitk_metric(self, metric: SimilarityMetric) -> sitk.ImageMetric:
        """
        ...
        if metric == SimilarityMetric.MEAN_SQUARES:
            return sitk.MeanSquaresImageToImageMetricv4()
        elif metric == SimilarityMetric.NORMALIZED_CORRELATION:
            return sitk.CorrelationImageToImageMetricv4()
        elif metric == SimilarityMetric.MUTUAL_INFORMATION:
            return sitk.JointHistogramMutualInformationImageToImageMetricv4()
        elif metric == SimilarityMetric.NORMALIZED_MUTUAL_INFO:
            metric = sitk.JointHistogramMutualInformationImageToImageMetricv4()
            metric.SetNormalizeJointHistogram(True)
            return metric
        elif metric == SimilarityMetric.MATTES_MUTUAL_INFO:
            return sitk.MattesMutualInformationImageToImageMetricv4()
        else:
    ...
    def register(self, fixed_image: Image, moving_image: Image) -> RegistrationResult:
        ...
        raise NotImplementedError("Phương thức register cần được triển khai trong lớp con")
    ...
    def _convert_from_sitk_image(self, sitk_image: sitk.Image, original_image: Image) -> Image:
        """
        ...
        image = Image(
            ...
            modality=original_image.modality,
            study_id=original_image.study_id,
            series_id=original_image.series_id,
            pixel_array=array,
            spacing=original_image.spacing,
            origin=original_image.origin,
            direction=original_image.direction,
            description=f"Transformed from {original_image.id}"
        )
...
class RigidRegistration(ImageRegistration):
    ...
    def __init__(self, parameters: RegistrationParameters = None):
        ...
        if parameters is None:
            parameters = RegistrationParameters(registration_type=RegistrationType.RIGID)
        else:
    ...
    def register(self, fixed_image: Image, moving_image: Image) -> RegistrationResult:
        ...
        try:
            ...
            registration_method.SetMetricSamplingPercentage(self.parameters.sampling_percentage)
            ...
            registration_method.SetSmoothingSigmasPerLevel(self.parameters.smoothing_sigmas)
...
class AffineRegistration(ImageRegistration):
    ...
    def __init__(self, parameters: RegistrationParameters = None):
        ...
        """
        if parameters is None:
            parameters = RegistrationParameters(registration_type=RegistrationType.AFFINE)
        else:
...
class BSplineDeformableRegistration(ImageRegistration):
    ...
    def __init__(self, parameters: RegistrationParameters = None):
        ...
        """
        if parameters is None:
            parameters = RegistrationParameters(registration_type=RegistrationType.DEFORMABLE_BSPLINE)
        else:
...
class DemonsDeformableRegistration(ImageRegistration):
    ...
    def __init__(self, parameters: RegistrationParameters = None):
        ...
        """
        if parameters is None:
            parameters = RegistrationParameters(registration_type=RegistrationType.DEFORMABLE_DEMONS)
        else:
...
def create_registration(registration_type: RegistrationType, 
                       parameters: RegistrationParameters = None) -> ImageRegistration:
    """
