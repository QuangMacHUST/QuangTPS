...
class DisplacementField:
    ...
    @classmethod
    def from_sitk_field(
        cls, sitk_field: sitk.Image, reference_image: Image = None
    ) -> "DisplacementField":
        """
    ...
    def from_registration_result(
        cls, registration_result, reference_image: Image
    ) -> "DisplacementField":
        """
    ...
    def compose(self, other: "DisplacementField") -> "DisplacementField":
        ...
        if (
            self.sitk_field.GetSize() != other.sitk_field.GetSize()
            or self.sitk_field.GetSpacing() != other.sitk_field.GetSpacing()
            or self.sitk_field.GetOrigin() != other.sitk_field.GetOrigin()
        ):
            raise DeformationError(
                "Không thể kết hợp hai trường chuyển dịch không tương thích"
            )
    ...
    def invert(self) -> "DisplacementField":
        ...
        try:
            ...
            raise DeformationError(
                f"Không thể tính nghịch đảo của trường chuyển dịch: {str(e)}"
            )
    ...
    def analyze_deformation(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "expansion_percentage": np.sum(expansion_regions)
            / expansion_regions.size
            * 100,
            "contraction_percentage": np.sum(contraction_regions)
            / contraction_regions.size
            * 100,
            "folding_percentage": np.sum(folding_regions) / folding_regions.size * 100,
        }

    def visualize(
        self, slice_index: int = None, downsample: int = 4, scale: float = 1.0
        ...
        # Tạo lưới tọa độ
        y, x = np.mgrid[
            0 : slice_field.shape[0] : downsample, 0 : slice_field.shape[1] : downsample
        ]
