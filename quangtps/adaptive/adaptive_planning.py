...
class AdaptiveEvaluationResult:
    ...
    def from_dict(cls, data: Dict[str, Any]) -> "AdaptiveEvaluationResult":
        ...
        result.change_types = [
            AnatomicalChangeType[ct] for ct in data.get("change_types", [])
        ]
        result.dose_impact = data.get("dose_impact", {})
        result.recommended_action = AdaptiveActionType[
            data.get("recommended_action", "CONTINUE_TREATMENT")
        ]
...
class PlanAdaptationSession:
    ...
    def __init__(
        self, patient: Patient, original_plan: TreatmentPlan, new_image: Image
    ):
        """
    ...
    @classmethod
    def from_dict(
        cls, data: Dict[str, Any], patient_db, plan_db, image_db
        ...
        if data["evaluation_result"]:
            session.evaluation_result = AdaptiveEvaluationResult.from_dict(
...
class AdaptivePlan:
    """
    Lớp đại diện cho một kế hoạch xạ trị thích ứng.
    Mở rộng từ kế hoạch điều trị cơ bản với các thông tin cụ thể cho adaptive planning.
    """

    def __init__(self, base_plan: TreatmentPlan, session: PlanAdaptationSession):
        """
        Khởi tạo kế hoạch thích ứng

        Parameters
        ----------
        base_plan : TreatmentPlan
            Kế hoạch điều trị cơ bản làm nền tảng
        session : PlanAdaptationSession
            Phiên adaptive planning tạo ra kế hoạch này
        """
        self.base_plan = base_plan
        self.session = session
        self.adaptation_type = (
            session.evaluation_result.recommended_action
            if session.evaluation_result
            else None
        )
        self.confidence_score = (
            session.evaluation_result.confidence_score
            if session.evaluation_result
            else 1.0
        )
    ...
    def approve(self, approver: str, notes: str = ""):
        """Phê duyệt kế hoạch thích ứng"""
        self.approved = True
        self.approver = approver
        self.approval_time = datetime.datetime.now()
        if notes:
            self.notes.append(f"Approved by {approver}: {notes}")

    def mark_delivered(self):
        """Đánh dấu kế hoạch đã được thực hiện"""
        self.delivered = True
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        return {
            "base_plan_id": self.base_plan.id,
            "session_id": self.session.session_id,
            "adaptation_type": self.adaptation_type.name
            if self.adaptation_type
            else None,
            "confidence_score": self.confidence_score,
...
class AdaptivePlanner:
    ...
    def create_adaptation_session(
        self, patient_id: str, plan_id: str, image_id: str
    ) -> PlanAdaptationSession:
        """
        ...
        if not patient or not original_plan or not new_image:
            raise AdaptivePlanningError(
                "Không thể tạo phiên thích ứng vì thiếu dữ liệu"
            )
        ...
        logger.info(
            f"Created adaptation session {session.session_id} for patient {patient_id}"
        )
        return session

    def evaluate_adaptation_need(
        self, session: PlanAdaptationSession, structures: List[Structure] = None
        ...
        if not structures:
            ...
            plan_structures = self.structure_db.get_structures_by_plan_id(
                session.original_plan.id
            )
        ...
        if result.change_types:
            ...
            # Đề xuất hành động
            if (
                AnatomicalChangeType.TARGET_POSITION in result.change_types
                and len(result.change_types) == 1
            ):
    ...
    def _evaluate_position_changes(
        self, structures: List[Structure], session: PlanAdaptationSession
    ) -> Dict[str, float]:
        """
    ...
    def _project_dose_to_new_image(self, session: PlanAdaptationSession) -> Dose:
        """
    ...
    def adapt_plan(
        self, session: PlanAdaptationSession, action_type: AdaptiveActionType = None
    ) -> TreatmentPlan:
        """
        ...
        if not session.evaluation_result:
            raise AdaptivePlanningError(
                "Cần phải đánh giá trước khi thích ứng kế hoạch"
            )
        ...
        if action_type == AdaptiveActionType.CONTINUE_TREATMENT:
            ...
            raise AdaptivePlanningError(
                f"Loại hành động không được hỗ trợ: {action_type}"
            )
    ...
    def _adapt_with_isocenter_shift(
        self, session: PlanAdaptationSession
    ) -> TreatmentPlan:
        """
        ...
        adapted_plan.description = (
            f"Adapted from {original_plan.id} with isocenter shift"
        )
    ...
    def _adapt_with_reoptimization(
        self, session: PlanAdaptationSession
    ) -> TreatmentPlan:
        """
    ...
    def _adapt_with_complete_replanning(
        self, session: PlanAdaptationSession
    ) -> TreatmentPlan:
        """
        ...
        adapted_plan.patient_id = session.patient.id
...
def create_adaptive_plan(base_plan, session=None, adaptation_type=None):
    """
    Convenience function để tạo AdaptivePlan

    Parameters
    ----------
    base_plan : TreatmentPlan
        Kế hoạch điều trị cơ bản
    session : PlanAdaptationSession, optional
        Phiên adaptive planning (tạo mock session nếu không cung cấp)
    adaptation_type : AdaptiveActionType, optional
        Loại thích ứng
    ...
    """
    if session is None:
        # Tạo mock session để testing
        from quangtps.core.types import Patient, Image

        mock_patient = Patient(patient_id="mock_patient", patient_name="Mock Patient")
        mock_image = Image()
        session = PlanAdaptationSession(mock_patient, base_plan, mock_image)

        if adaptation_type:
            result = AdaptiveEvaluationResult()
            result.recommended_action = adaptation_type
            session.evaluation_result = result

    return AdaptivePlan(base_plan, session)
