...
class StructureDatabase:
    ...
    def create_structure(
        self, study_id, name, structure_type=None, color=None, metadata=None
    ):
        """
    ...
    def update_structure(
        self, structure_id, name=None, structure_type=None, color=None, metadata=None
    ):
        """
        ...
        try:
            ...
            if not current_structure:
                logger.warning(
                    f"Không thể cập nhật cấu trúc không tồn tại: {structure_id}"
                )
                return False
    ...
    def delete_structure(self, structure_id):
        ...
        try:
            ...
            self.db.execute_transaction(
                [
                    (
                        "DELETE FROM structure_points WHERE structure_id = ?",
                        (structure_id,),
                    ),
    ...
    def search_structures(
        self, study_id=None, name=None, structure_type=None, limit=100, offset=0
    ):
        """
    ...
    def add_structure_contour(
        self, structure_id, slice_index, contour_points, metadata=None
    ):
        """
        ...
        try:
            ...
            self.db.execute_query(query, params)
            logger.info(
                f"Đã thêm đường viền mới cho cấu trúc {structure_id} tại lát cắt {slice_index}"
            )
    ...
    def delete_structure_contour(self, structure_id, slice_index=None):
        ...
        try:
            if slice_index is not None:
                ...
                logger.info(
                    f"Đã xóa đường viền của cấu trúc {structure_id} tại lát cắt {slice_index}"
                )
            else:
    ...
    def import_structure_from_dicom(self, study_id, structure_dict, contours=None):
        ...
        try:
            ...
            existing_structures = self.search_structures(
                study_id=study_id, name=structure_dict.get("name")
            )
            ...
            if contours and isinstance(contours, dict):
                ...
                logger.info(
                    f"Đã import {len(contours)} đường viền cho cấu trúc {structure_id}"
                )
    ...
    def import_structure_from_imaging(
        self, study_id, imaging_structure, add_to_set_id=None
    ):
        """
        ...
        try:
            ...
            segmentation_structure = imaging_to_segmentation_structure(
                imaging_structure
            )
            ...
            if add_to_set_id:
                ...
                logger.info(
                    "Added structure %s to structure set %s",
                    structure_id,
                    add_to_set_id,
                )
            ...
            logger.error(
                "Error importing structure from imaging: %s", str(e), exc_info=True
            )
            raise DatabaseError(
                f"Failed to import structure from imaging: {str(e)}"
            ) from e
    ...
    def save_structure_set(self, structure_set: StructureSet) -> str:
        ...
        try:
            ...
            metadata = {
                ...
                "creation_date": structure_set.creation_date.isoformat()
                if hasattr(structure_set, "creation_date")
                else current_time,
                "modified_date": structure_set.modified_date.isoformat()
                if hasattr(structure_set, "modified_date")
                else current_time,
            ...
            if exists:
                ...
                self.db.execute_query(
                    query,
                    (structure_set.name, metadata_json, current_time, structure_set.id),
                )
    ...
    def get_patient_structures(self, patient_id: str) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            if not results:
                logger.info(
                    "Không tìm thấy cấu trúc nào cho bệnh nhân có ID: %s", patient_id
                )
                return []
            ...
            for row in results:
                ...
                if "metadata" in structure_dict and structure_dict["metadata"]:
                    try:
                        structure_dict["metadata"] = json.loads(
            ...
            logger.info(
                "Đã lấy %d cấu trúc cho bệnh nhân có ID: %s",
                len(structures),
                patient_id,
            )
            return structures
    ...
    def get_patient_structure_sets(self, patient_id: str) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            if not results:
                logger.info(
                    "Không tìm thấy tập hợp cấu trúc nào cho bệnh nhân có ID: %s",
                    patient_id,
                )
                return []
            ...
            for row in results:
                ...
                if "metadata" in structure_set_dict and structure_set_dict["metadata"]:
                    try:
                        structure_set_dict["metadata"] = json.loads(
            ...
            logger.info(
                "Đã lấy %d tập hợp cấu trúc cho bệnh nhân có ID: %s",
                len(structure_sets),
                patient_id,
            )
            ...
            logger.error(
                "Lỗi khi lấy danh sách tập hợp cấu trúc: %s", str(e), exc_info=True
            )
            raise DatabaseError(
                "Lỗi khi lấy danh sách tập hợp cấu trúc: %s" % str(e)
            ) from e

    def delete_structure_set(
        self, structure_set_id: str, delete_structures: bool = False
    ) -> bool:
        """
        ...
        try:
            ...
            if not result:
                logger.warning(
                    "Không thể xóa tập hợp cấu trúc không tồn tại: %s", structure_set_id
                )
                return False
            ...
            if delete_structures:
                ...
                structure_ids = self.db.execute_query(
                    query, (structure_set_id,), fetchall=True
                )
    ...
    def get_structure_set(self, structure_set_id):
        ...
        try:
            ...
            if not result:
                logger.warning(
                    "Không tìm thấy tập hợp cấu trúc có ID: %s", structure_set_id
                )
                return None
            ...
            logger.error(
                "Lỗi khi lấy thông tin tập hợp cấu trúc: %s", str(e), exc_info=True
            )
            raise DatabaseError(
                f"Không thể lấy thông tin tập hợp cấu trúc: {str(e)}"
            ) from e
