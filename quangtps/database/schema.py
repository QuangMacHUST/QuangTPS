...
SCHEMA_TABLES = {
    ...
    "study": {
        "columns": {
            ...
            "patient_id": {
                "type": DataType.TEXT,
                "constraints": ["REFERENCES patient(patient_id)"],
            },
    ...
    "series": {
        "columns": {
            ...
            "study_id": {
                "type": DataType.TEXT,
                "constraints": ["REFERENCES study(study_id)"],
            },
    ...
    "image": {
        "columns": {
            ...
            "series_id": {
                "type": DataType.TEXT,
                "constraints": ["REFERENCES series(series_id)"],
            },
    ...
    "contour": {
        "columns": {
            ...
            "structure_id": {
                "type": DataType.TEXT,
                "constraints": ["REFERENCES structure(structure_id)"],
            },
    ...
    "plan": {
        "columns": {
            ...
            "approval_status": {
                "type": DataType.TEXT
            },  # Draft, Under Review, Approved, etc.
    ...
    "beam": {
        "columns": {
            ...
            "plan_id": {
                "type": DataType.TEXT,
                "constraints": ["REFERENCES plan(plan_id)"],
            },
            "name": {"type": DataType.TEXT, "constraints": [Constraint.NOT_NULL]},
            "description": {"type": DataType.TEXT},
    ...
    "fluence_map": {
        "columns": {
            ...
            "beam_id": {
                "type": DataType.TEXT,
                "constraints": ["REFERENCES beam(beam_id)"],
            },
    ...
    "dvh": {
        "columns": {
            ...
            "dose_id": {
                "type": DataType.TEXT,
                "constraints": ["REFERENCES dose(dose_id)"],
            },
            "structure_id": {
                "type": DataType.TEXT,
                "constraints": ["REFERENCES structure(structure_id)"],
    ...
    "dose_objective": {
        "columns": {
            ...
            "type": {
                "type": DataType.TEXT
            },  # Max Dose, Min Dose, Mean Dose, D95, V20, etc.
        ...
        if constraints:
            ...
            for constraint in constraints:
                if isinstance(constraint, Constraint):
                    ...
                    and constraint[0] == Constraint.DEFAULT
                ):
                    # Default values
...
def validate_data_for_table(table_name: str, data: Dict[str, Any]) -> Dict[str, Any]:
    ...
    for col_name, col_props in columns.items():
        ...
        if Constraint.NOT_NULL in constraints and col_name not in data:
            raise ValueError(
                f"Column '{col_name}' is required for table '{table_name}'"
            )
    ...
    for key, value in data.items():
        if key in columns:
            ...
            logger.warning(
                f"Column '{key}' not defined for table '{table_name}', ignoring"
            )
