...
class ImageDatabase:
    ...
    def save_image(
        ...
        try:
            ...
            with h5py.File(series_path, "w") as f:
                ...
                f.create_dataset(
                    "data", data=image.data, compression="gzip", compression_opts=9
                )
    ...
    def get_patient_images(self, patient_id: str) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            if not results:
                logger.info(
                    "Không tìm thấy hình ảnh nào cho bệnh nhân có ID: %s", patient_id
                )
                return []
            ...
            logger.info(
                "Đã lấy %d chuỗi hình ảnh cho bệnh nhân có ID: %s",
                len(series_list),
                patient_id,
            )
            return series_list
