...
class Material:
    ...
    def _calculate_radiation_length(self) -> float:
        """
        Calculate the radiation length for this material.
        
        Radiation length is the mean distance over which a high-energy electron
        loses all but 1/e of its energy by bremsstrahlung.
        
        Returns:
            Radiation length in cm
        """
        # Simplified calculation based on effective Z
        # More accurate calculation would use the full Tsai formula
        if self.effective_z < 2:
            return 63.0  # Approximate value for hydrogen
        elif self.effective_z < 6:
            return 36.1  # Approximate value for carbon
        elif self.effective_z < 10:
            return 24.0  # Approximate value for water
        elif self.effective_z < 15:
            return 12.5  # Approximate value for soft tissue/muscle
        elif self.effective_z < 20:
            return 2.5   # Approximate value for bone
        else:
            return 0.5   # Approximate value for high-Z materials like metal implants
