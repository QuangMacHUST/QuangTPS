...
class DrawingTool:
    ...
    def apply(self, image: np.ndarray, position: Tuple[int, int], 
...
class ThresholdTool(DrawingTool):
    ...
    def apply(self, image: np.ndarray, ct_image: np.ndarray, *args, **kwargs) -> np.ndarray:
        """
        ...
        _, threshold_mask = cv2.threshold(
            ...
            cv2.THRESH_BINARY if self.min_threshold <= self.max_threshold else cv2.THRESH_BINARY_INV
        )
...
class SmartBrushTool(BrushTool):
    ...
    def apply(self, image: np.ndarray, position: Tuple[int, int],
              ct_image: np.ndarray, value: int = 1) -> np.ndarray:
        """
        ...
        threshold_mask = np.logical_and(
            ct_image >= lower_bound,
            ct_image <= upper_bound
        ).astype(np.uint8)
...
class InterpolateTool(DrawingTool):
    ...
    def interpolate(self, contours: Dict[int, np.ndarray], 
    ...
    def _linear_interpolation(self, mask1: np.ndarray, mask2: np.ndarray, 
