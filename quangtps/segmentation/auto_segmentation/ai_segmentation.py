...
class AISegmentationWidget(QWidget):
    ...
    def start_segmentation(self) -> None:
        ...
        if not model:
            QMessageBox.warning(self, "No Model", "No AI model is currently active.")
            return
    ...
    def handle_segmentation_complete(self, results: Dict[str, np.ndarray]) -> None:
        ...
        Args:
            results: Dictionary of structure types to segmentation masks
        """
        # Enable the UI
        ...
        if not results:
            self.status_label.setText(
                "Segmentation completed, but no structures were generated"
            )
            logger.warning("Segmentation completed without any results")
            return

        # Create structures from masks
        structures = {}
        for structure_type, mask in results.items():
            try:
                # Generate a consistent color based on structure type
                color_hash = sum(ord(c) for c in structure_type) % 360
                color = QColor()
                color.setHsv(color_hash, 200, 200)

                # Auto-name if enabled
                structure_name = structure_type
                if self.auto_name_checkbox.isChecked():
                    # Add prefix based on side if applicable
                    if "Left" in structure_type:
                        structure_name = f"L_{structure_type.replace(' Left', '')}"
                    elif "Right" in structure_type:
                        structure_name = f"R_{structure_type.replace(' Right', '')}"

                # Create the structure with smoothing if enabled
                smoothing = self.smoothing_slider.value() / 100.0
                structure = create_structure_from_mask(mask, structure_name, color)

                if smoothing > 0:
                    try:
                        # Apply smoothing if available
                        from quangtps.segmentation.contour.smoothing import (
                            apply_smoothing,
                        )

                        structure = apply_smoothing(structure, smoothing)
                    except ImportError:
                        logger.warning(
                            f"Smoothing unavailable, skipping for {structure_name}"
                        )

                structures[structure_type] = structure
                logger.info(
                    f"Created structure '{structure_name}' from AI segmentation"
                )
            except Exception as e:
                logger.error(f"Error creating structure {structure_type}: {e}")
                # Continue with other structures

        num_structures = len(structures)
        self.status_label.setText(
            f"Segmentation complete. Created {num_structures} structures."
        )

        # Emit signal with the created structures
        self.segmentationComplete.emit(structures)

        # Show auto-review if enabled
        if self.auto_review_checkbox.isChecked() and structures:
            try:
                from quangtps.segmentation.validation.structure_review import (
                    StructureReviewDialog,
                )

                review_dialog = StructureReviewDialog(structures.values(), parent=self)
                review_dialog.show()
            except ImportError as e:
                logger.warning(f"Cannot show structure review: {e}")
                QMessageBox.information(
                    self,
                    "Segmentation Complete",
                    f"Successfully segmented {num_structures} structures.",
                )
    ...
    def handle_segmentation_failed(self, error_message: str) -> None:
        ...
        # Show error message
        QMessageBox.warning(
            self, "Segmentation Failed", f"Segmentation failed: {error_message}"
        )
...
def create_structure_from_mask(
    mask: np.ndarray, name: str, color: Optional[QColor] = None
) -> "Structure":
    """
