...
class ClinicalGoal:
    ...
    def __init__(
        ...
        value: float,
        priority: GoalPriority = GoalPriority.MAJOR,
    ...
    def __str__(self):
        ...
        if self.goal_type == GoalType.VOLUME_AT_DOSE:
            ...
            return (
                f"{self.structure_name} D{self.volume_level}% {op_str} {self.value}Gy"
            )
        else:
    ...
    def evaluate(self, dvh_calculator) -> GoalResult:
        ...
        try:
            ...
            if self.goal_type == GoalType.VOLUME_AT_DOSE:
                ...
                self.achieved_value = dvh_calculator.get_homogeneity_index(
                    self.structure_id
                )
...
class ClinicalGoalTemplate:
    ...
    def __init__(self, name, treatment_site, description=""):
        ...
        self.description = description
        self.goal_collection = ClinicalGoalCollection(
    ...
    def save_template(self, template):
        ...
        try:
            ...
            filename = os.path.join(
                self.templates_dir, f"{template.name.lower().replace(' ', '_')}.json"
            )
...
# Helper functions for creating common clinical goals
...
def create_d_goal(
    ...
    """
    Tạo mục tiêu liều tại thể tích (D goal).
    ...
    """
    return ClinicalGoal(
        structure_id=structure_name.lower().replace(" ", "_"),
        structure_name=structure_name,
        goal_type=GoalType.DOSE_AT_VOLUME,
        operator=operator,
        value=dose_gy,
        priority=priority,
        volume_level=volume_percent,
        notes=notes,
    )
...
def create_v_goal(
    ...
    """
    Tạo mục tiêu thể tích tại liều (V goal).
...
def create_max_dose_goal(
    structure_name: str,
    dose_gy: float,
    operator: GoalOperator = GoalOperator.LESS_THAN,
    priority: GoalPriority = GoalPriority.MAJOR,
    notes: str = "",
) -> ClinicalGoal:
    """
    Tạo mục tiêu liều tối đa.
    ...
    dose_gy : float
        Liều tối đa (Gy)
    operator : GoalOperator, optional
        Toán tử so sánh
    priority : GoalPriority, optional
        Mức độ ưu tiên
    notes : str, optional
...
def create_min_dose_goal(
    structure_name: str,
    dose_gy: float,
    operator: GoalOperator = GoalOperator.GREATER_THAN,
    priority: GoalPriority = GoalPriority.MAJOR,
    notes: str = "",
) -> ClinicalGoal:
    """
    Tạo mục tiêu liều tối thiểu.
