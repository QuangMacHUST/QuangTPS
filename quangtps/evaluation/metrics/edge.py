...
class EdgeAnalysis:
    ...
    def calculate_edge_width(
        ...
        distance_map = ndimage.distance_transform_edt(
            ~high_dose_mask, sampling=voxel_size_mm
        )
    ...
    def calculate_dose_falloff(
        ...
        distance_map = ndimage.distance_transform_edt(
            ~target_mask, sampling=voxel_size_mm
        )
        ...
        percentage_falloff = (
            100 * (1 - distance_dose / edge_dose) if edge_dose > 0 else 0.0
        )
    ...
    @staticmethod
    def analyze_dose_gradient_orientations(
        dose_array: np.ndarray,
        target_mask: np.ndarray,
        voxel_size_mm: List[float] = None,
    ...
    def calculate_edge_conformity(
        ...
        distance_map_inside = ndimage.distance_transform_edt(
            target_mask, sampling=voxel_size_mm
        )
        distance_map_outside = ndimage.distance_transform_edt(
            ~target_mask, sampling=voxel_size_mm
        )
        ...
        v_target_edge_covered = (
            np.sum(target_edge_mask & reference_dose_mask) * voxel_volume
        )
        ...
        v_outside_edge_dose = (
            np.sum(outside_edge_mask & reference_dose_mask) * voxel_volume
        )

        # Tính chỉ số đồng dạng biên
        target_edge_coverage = (
            v_target_edge_covered / v_target_edge if v_target_edge > 0 else 0.0
        )
        outside_edge_sparing = (
            1.0 - (v_outside_edge_dose / v_outside_edge) if v_outside_edge > 0 else 1.0
        )
    ...
    @staticmethod
    def identify_edge_irregularities(
        dose_array: np.ndarray,
        target_mask: np.ndarray,
        reference_dose: float,
        ...
        if np.sum(edge_region_mask) == 0:
            return {"irregularity_score": 0.0, "irregularities": []}
            ...
            irregularities.append(
                {
                    ...
                    "dose_percent": float(mean_dose / reference_dose * 100)
                    if reference_dose > 0
                    else 0.0,
        ...
        irregularity_score = (
            num_features * (std_gradient / mean_gradient) if mean_gradient > 0 else 0.0
        )
    ...
    @staticmethod
    def generate_edge_analysis_report(
        dose_array: np.ndarray,
        target_mask: np.ndarray,
        reference_dose: float,
        voxel_size_mm: List[float] = None,
    ...
    def interpret_edge_results(report: Dict) -> Dict[str, str]:
        ...
        if edge_width < 2:
            ...
            interpretations["edge_width"] = (
                "Biên liều dốc, độ chuyển tiếp liều phù hợp."
            )
        elif edge_width < 10:
            interpretations["edge_width"] = (
                "Biên liều trung bình, độ chuyển tiếp liều vừa phải."
            )
        else:
        ...
        if percentage_falloff > 80:
            interpretations["dose_falloff"] = (
                "Độ giảm liều rất tốt, bảo vệ tối ưu cho mô lành."
            )
        elif percentage_falloff > 60:
            interpretations["dose_falloff"] = (
                "Độ giảm liều tốt, bảo vệ đáng kể cho mô lành."
            )
        elif percentage_falloff > 40:
            interpretations["dose_falloff"] = (
                "Độ giảm liều trung bình, cần xem xét cải thiện."
            )
        else:
            interpretations["dose_falloff"] = (
                "Độ giảm liều thấp, cần cải thiện để bảo vệ mô lành tốt hơn."
            )

        # Diễn giải đồng dạng biên
        edge_conformity = report["edge_conformity"]["edge_conformity_index"]
        if edge_conformity > 0.9:
            interpretations["edge_conformity"] = (
                "Đồng dạng biên liều xuất sắc, phân phối liều rất phù hợp với hình dạng mục tiêu."
            )
        elif edge_conformity > 0.8:
            interpretations["edge_conformity"] = (
                "Đồng dạng biên liều tốt, phân phối liều phù hợp với hình dạng mục tiêu."
            )
        elif edge_conformity > 0.7:
            interpretations["edge_conformity"] = (
                "Đồng dạng biên liều chấp nhận được, có thể cải thiện thêm."
            )
        else:
            interpretations["edge_conformity"] = (
                "Đồng dạng biên liều kém, cần cải thiện sự phù hợp giữa phân phối liều và hình dạng mục tiêu."
            )

        # Diễn giải các bất thường
        num_irregularities = report["edge_irregularities"]["num_irregularities"]
        if num_irregularities == 0:
            interpretations["irregularities"] = (
                "Không phát hiện bất thường tại biên liều."
            )
        elif num_irregularities < 3:
            interpretations["irregularities"] = (
                f"Phát hiện {num_irregularities} vùng bất thường nhỏ tại biên liều."
            )
        elif num_irregularities < 6:
            interpretations["irregularities"] = (
                f"Phát hiện {num_irregularities} vùng bất thường đáng chú ý tại biên liều, cần xem xét."
            )
        else:
            interpretations["irregularities"] = (
                f"Phát hiện {num_irregularities} vùng bất thường đáng kể tại biên liều, cần đánh giá kỹ và cải thiện kế hoạch."
            )
        ...
        if average_score > 0.9:
            interpretations["overall"] = (
                "Đánh giá biên liều: Xuất sắc. Biên liều rõ ràng, dốc và đồng dạng tốt với mục tiêu."
            )
        elif average_score > 0.8:
            interpretations["overall"] = (
                "Đánh giá biên liều: Rất tốt. Biên liều có đặc tính phù hợp cho mục đích điều trị."
            )
        elif average_score > 0.7:
            interpretations["overall"] = (
                "Đánh giá biên liều: Tốt. Biên liều đáp ứng các yêu cầu cơ bản nhưng có thể cải thiện."
            )
        elif average_score > 0.6:
            interpretations["overall"] = (
                "Đánh giá biên liều: Khá. Biên liều có một số hạn chế cần được xem xét."
            )
        else:
            interpretations["overall"] = (
                "Đánh giá biên liều: Cần cải thiện. Biên liều không đáp ứng tốt các tiêu chí mong muốn."
            )
...
def calculate_edge_metrics(
    ...
    """
    Alias function cho generate_edge_analysis_report
    ...
    -------
    Dict[str, Union[float, Dict]]
        Dictionary chứa các chỉ số edge analysis
    """
    # Chuyển đổi spacing từ tuple sang list
    voxel_size_mm = list(spacing)

    # Sử dụng EdgeAnalysis class để tính toán
    edge_analysis = EdgeAnalysis()

    return edge_analysis.generate_edge_analysis_report(
        dose_grid, target_mask, prescription_dose, voxel_size_mm
    )


__all__ = ["EdgeAnalysis", "calculate_edge_metrics"]
