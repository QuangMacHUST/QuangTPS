...
class NTCP:
    ...
    def critical_volume_model(
        ...
        if volume_fraction > critical_volume_fraction:
            fsv = 1.0 - (volume_fraction - critical_volume_fraction) / (
                1.0 - critical_volume_fraction
            )
...
class EUD:
    ...
    def calculate(
        self, dose: np.ndarray, a: float, voxel_volume: Union[float, np.ndarray] = 1.0
    ) -> float:
        """
    ...
    def calculate_from_dvh(
        self, dose_bins: np.ndarray, volume_bins: np.ndarray, a: float
    ) -> float:
        """
...
class BED:
    ...
    @staticmethod
    def calculate(
        total_dose: float, dose_per_fraction: float, alpha_beta: float
    ) -> float:
        """
    ...
    @staticmethod
    def convert_bed_to_physical(
        bed: float, dose_per_fraction: float, alpha_beta: float
    ) -> float:
        """
    ...
    @staticmethod
    def calculate_bed_distribution(
        dose: np.ndarray, fractionation: Fractionation, alpha_beta: float
    ) -> np.ndarray:
        """
    ...
    @staticmethod
    def calculate_eqd2_distribution(
        dose: np.ndarray, fractionation: Fractionation, alpha_beta: float
    ) -> np.ndarray:
        """
...
def calculate_radiobiological_metrics(
    ...
    """
    Tính toán các metrics sinh học từ dose grid và structure masks.
    ...
    dose_grid : np.ndarray
        Dose distribution in Gy
    target_masks : Dict[str, np.ndarray]
        Dictionary of target structure masks
    oar_masks : Dict[str, np.ndarray], optional
        Dictionary of OAR structure masks
    tcp_params : Dict[str, Dict[str, float]], optional
        TCP parameters for each target structure
    ntcp_params : Dict[str, Dict[str, float]], optional
        NTCP parameters for each OAR
    spacing : tuple
        Voxel spacing in mm
    ...
    -------
    Dict[str, Dict[str, float]]
        Dictionary containing TCP and NTCP values for each structure
    """
    logger.info(f"Calculating radiobiological metrics for {len(target_masks)} targets")
    ...
    # Default TCP parameters if not provided
    if tcp_params is None:
        tcp_params = {
            "PTV": {"d50": 50.0, "gamma50": 2.0, "alpha": 0.3, "beta": 0.03},
        }

    # Default NTCP parameters if not provided
    if ntcp_params is None:
        ntcp_params = {
            "Spinal_Cord": {"td50": 50.0, "m": 0.175, "n": 0.05},
            "Lung": {"td50": 20.0, "m": 0.18, "n": 1.0},
            "Heart": {"td50": 45.0, "m": 0.10, "n": 0.35},
            "Rectum": {"td50": 80.0, "m": 0.15, "n": 0.12},
        }

    # Calculate TCP for target structures
    for structure_name, mask in target_masks.items():
        if structure_name in tcp_params:
            params = tcp_params[structure_name]
            structure_dose = dose_grid[mask > 0]

            if len(structure_dose) > 0:
                # Calculate TCP using different models
                tcp_poisson = TCP.poisson_model(
                    structure_dose, params["d50"], params["gamma50"], voxel_volume
                )

                tcp_logistic = TCP.logistic_model(
                    structure_dose, params["d50"], params["gamma50"], voxel_volume
                )

                if "alpha" in params and "beta" in params and fractionation:
                    tcp_lq = TCP.lq_model(
                        structure_dose,
                        params["alpha"],
                        params["beta"],
                        fractionation.num_fractions,
                        voxel_volume,
                    )
                else:
                    tcp_lq = tcp_poisson  # Fallback
                ...
                logger.info(
                    f"TCP calculated for {structure_name}: Poisson={tcp_poisson:.3f}"
                )

    # Calculate NTCP for OAR structures
    if oar_masks:
        for structure_name, mask in oar_masks.items():
            if structure_name in ntcp_params:
                params = ntcp_params[structure_name]
                structure_dose = dose_grid[mask > 0]

                if len(structure_dose) > 0:
                    # Calculate NTCP using LKB model
                    ntcp_lkb = NTCP.lkb_model(
                        structure_dose,
                        params["td50"],
                        params["m"],
                        params["n"],
                    ...
                    # Calculate EUD
                    eud_calculator = EUD()
                    eud = eud_calculator.calculate(
                        structure_dose, 1 / params["n"], voxel_volume
                    )

                    if structure_name not in results:
                        results[structure_name] = {}

                    results[structure_name].update({"ntcp_lkb": ntcp_lkb, "eud": eud})

                    logger.info(
                        f"NTCP calculated for {structure_name}: LKB={ntcp_lkb:.3f}"
                    )

    # Calculate BED if fractionation is provided
    if fractionation:
        for structure_name in results.keys():
            # Use typical alpha/beta values
            alpha_beta = 10.0 if "PTV" in structure_name else 3.0

            dose_per_fraction = np.mean(dose_grid) / fractionation.num_fractions
            bed = BED.calculate(np.mean(dose_grid), dose_per_fraction, alpha_beta)
            eqd2 = BED.calculate_eqd2(np.mean(dose_grid), dose_per_fraction, alpha_beta)

            results[structure_name].update({"bed": bed, "eqd2": eqd2})

    logger.info(f"Radiobiological metrics calculated for {len(results)} structures")
    return results


# Alias functions for compatibility
calculate_biological_indices = calculate_radiobiological_metrics
