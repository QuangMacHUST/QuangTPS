...
class IntegralDoseAnalysis:
    ...
    @staticmethod
    def calculate_integral_dose(dose_array: np.ndarray, 
    ...
    @staticmethod
    def calculate_mean_dose(dose_array: np.ndarray, 
    ...
    @staticmethod
    def calculate_structure_volume(structure_mask: np.ndarray, 
                                voxel_volume_cc: float = 0.001) -> float:
        """
    ...
    @staticmethod
    def calculate_multiple_integral_doses(dose_array: np.ndarray, 
                                       structure_masks: Dict[str, np.ndarray],
                                       voxel_volume_cc: float = 0.001,
                                       densities: Dict[str, float] = None) -> Dict[str, Dict[str, float]]:
        """
        ...
        for name, mask in structure_masks.items():
            ...
            # Tính thể tích
            volume = IntegralDoseAnalysis.calculate_structure_volume(mask, voxel_volume_cc)
            
            # Tính phần trăm của tổng liều tích phân
            percent_of_total = (integral_dose / total_integral_dose * 100) if total_integral_dose > 0 else 0
    ...
    @staticmethod
    def calculate_normal_tissue_integral_dose(dose_array: np.ndarray,
                                           target_mask: np.ndarray,
                                           body_mask: Optional[np.ndarray] = None,
                                           voxel_volume_cc: float = 0.001,
                                           target_density: float = 1.0,
                                           normal_tissue_density: float = 1.0) -> Dict[str, float]:
        """
        ...
        # Tính liều trung bình
        target_mean_dose = IntegralDoseAnalysis.calculate_mean_dose(dose_array, target_mask)
        ...
        normal_tissue_percent = (normal_tissue_id / total_id * 100) if total_id > 0 else 0
    ...
    @staticmethod
    def calculate_integral_dose_ratio(target_id: float, normal_tissue_id: float) -> float:
        """
    ...
    @staticmethod
    def calculate_selective_integral_dose(target_mean_dose: float, 
                                       normal_tissue_mean_dose: float) -> float:
        """
    ...
    @staticmethod
    def generate_integral_dose_report(integral_dose_data: Dict[str, Dict[str, float]]) -> str:
        """
        ...
        report += "-" * 80 + "\n"
        report += f"{'TỔNG':<20} {integral_dose_data['total']['integral_dose_J']:<15.2f} "
    ...
    def interpret_normal_tissue_dose(normal_tissue_data: Dict[str, float]) -> str:
        ...
        id_ratio = IntegralDoseAnalysis.calculate_integral_dose_ratio(target_id, normal_tissue_id)
        ...
        if normal_tissue_percent < 30:
            ...
            dose_distribution = "trung bình, phân bố liều giữa mục tiêu và mô lành khá cân bằng"
        elif normal_tissue_percent < 70:
            dose_distribution = "kém, phần lớn liều nằm ngoài thể tích mục tiêu"
        else:
            dose_distribution = "rất kém, liều tích phân chủ yếu nằm ngoài thể tích mục tiêu"
