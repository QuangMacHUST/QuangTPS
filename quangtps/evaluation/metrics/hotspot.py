...
class HotspotAnalysis:
                    ...
                    hotspots.append(
                        {
                            ...
                            "percent_over_reference": float(
                                (max_dose / reference_dose - 1) * 100
                            ),
                        }
                    )
                    ...
                    coldspots.append(
                        {
                            ...
                            "mean_dose": float(mean_dose),
                            "location": tuple(center_coords),
                            "percent_under_reference": float(
                                (1 - min_dose / reference_dose) * 100
                            ),
                        }
                    )
    ...
    @staticmethod
    def calculate_hotspot_volume_histograms(
        dose_array: np.ndarray,
        reference_dose: float,
    ...
    @staticmethod
    def calculate_coldspot_volume_histograms(
        dose_array: np.ndarray,
        reference_dose: float,
        thresholds: List[float] = None,
        voxel_size_cc: float = 0.001,
        ...
        for threshold in thresholds:
            ...
            if target_mask is not None:
                # Chỉ xét trong thể tích mục tiêu
                volume = (
                    np.sum((dose_array < threshold_dose) & target_mask) * voxel_size_cc
                )
            else:
                # Xét toàn bộ mảng liều
                volume = np.sum(dose_array < threshold_dose) * voxel_size_cc
    ...
    @staticmethod
    def calculate_relative_hotspots(
        dose_array: np.ndarray,
        reference_dose: float,
    ...
    @staticmethod
    def generate_spots_report(
        hotspots: List[Dict], coldspots: List[Dict], reference_dose: float
    ) -> Dict:
        """
        ...
        report = {
            ...
            "hotspots": {
                ...
                "max_dose_percent": (
                    max([h["max_dose"] for h in hotspots]) / reference_dose * 100
                )
                if hotspots
                else 0,
                "details": hotspots,
            ...
            "coldspots": {
                ...
                "min_dose_percent": (
                    min([c["min_dose"] for c in coldspots]) / reference_dose * 100
                )
                if coldspots
                else 0,
                "details": coldspots,
    ...
    def interpret_hotspots(hotspots: List[Dict], reference_dose: float) -> str:
        ...
        return (
            f"Phát hiện {len(hotspots)} điểm nóng với tổng thể tích {total_volume:.2f} cm³. "
    ...
    def interpret_coldspots(coldspots: List[Dict], reference_dose: float) -> str:
        ...
        return (
            f"Phát hiện {len(coldspots)} điểm lạnh với tổng thể tích {total_volume:.2f} cm³. "
...
def calculate_hotspot_metrics(
    ...
    hotspot_threshold: float = 110.0,
    coldspot_threshold: float = 90.0,
    spacing: tuple = (1.0, 1.0, 1.0),
) -> Dict[str, Union[List[Dict], Dict[str, float]]]:
    """
    Alias function cho generate_spots_report
    ...
    dose_grid : np.ndarray
        Ma trận liều 3D
    target_mask : np.ndarray, optional
        Mask của target structure (dùng cho coldspot analysis)
    prescription_dose : float, optional
        Liều kê toa (nếu None, sử dụng D50 trong target)
    hotspot_threshold : float, optional
        Ngưỡng phần trăm để xác định hotspot (mặc định 110%)
    coldspot_threshold : float, optional
        Ngưỡng phần trăm để xác định coldspot (mặc định 90%)
    ...
    -------
    Dict[str, Union[List[Dict], Dict[str, float]]]
        Dictionary chứa kết quả phân tích hotspot và coldspot
    """
    # Tính toán voxel volume
    voxel_volume = spacing[0] * spacing[1] * spacing[2] / 1000.0  # cm³

    # Xác định reference dose
    if prescription_dose is None:
        if target_mask is not None:
            target_doses = dose_grid[target_mask]
            reference_dose = np.percentile(target_doses, 50)  # D50
        else:
            reference_dose = np.percentile(dose_grid, 50)
    else:
        reference_dose = prescription_dose
    ...
    # Tạo báo cáo
    return HotspotAnalysis.generate_spots_report(hotspots, coldspots, reference_dose)


__all__ = ["HotspotAnalysis", "calculate_hotspot_metrics"]
