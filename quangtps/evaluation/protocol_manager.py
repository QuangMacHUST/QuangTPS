...
class ProtocolManager:
            ...
            for filename in os.listdir(self.protocols_dir):
                if filename.endswith(".json"):
                    try:
                        ...
                        logger.error(
                            f"Error loading protocol from {filename}: {str(e)}"
                        )

            logger.info(
                f"Loaded {len(self.protocols)} protocols from {self.protocols_dir}"
            )
    ...
    def create_custom_protocol(
        self, name: str, site: str, description: str = ""
    ) -> ClinicalProtocol:
        """
...
if __name__ == "__main__":
    ...
    # Add goals
    new_protocol.goals.append(protocol_manager.create_goal("PTV", "D95", 95.0, "high"))
