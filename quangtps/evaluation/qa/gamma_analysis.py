...
class GammaAnalysis:
    ...
    def compute(self, mask: Optional[Union[np.ndarray, sitk.Image]] = None) -> np.ndarray:
        """
        ...
        if np.sum(eval_mask) > 0:
            self.passing_rate = np.sum(gamma_map[eval_mask] <= 1.0) / np.sum(eval_mask) * 100.0
        else:
            self.passing_rate = 0.0
        
        # Tính histogram
        self.histogram = np.histogram(gamma_map[eval_mask], bins=100, range=(0, max_gamma))
    ...
    def plot_gamma_map(self, slice_idx=None, ax=None, figsize=(10, 8), colormap='viridis', show_colorbar=True):
        """
        Vẽ bản đồ Gamma.
        ...
        im = ax.imshow(gamma_slice, cmap=colormap, vmin=0, vmax=min(2.0, self.parameters.max_gamma))
        
        if show_colorbar:
            cbar = plt.colorbar(im, ax=ax)
            cbar.set_label('Gamma Value')
        
        ax.set_title(f'Gamma Map (Slice {slice_idx}, Pass Rate: {self.passing_rate:.2f}%)')
    ...
    def __str__(self) -> str:
        ...
        return (f"Phân tích Gamma ({self.parameters.dose_threshold}{self.parameters.dose_threshold_unit}/"
