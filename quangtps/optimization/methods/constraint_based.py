...
from quangtps.optimization.objectives import ObjectiveCollection, ObjectiveFunction
...
class ConstraintBasedMethod:
    ...
    def create_engine(
        self, solver_name: str = "gradient_descent"
    ) -> OptimizationEngine:
        """
...
class ConstraintSatisfactionMethod(ConstraintBasedMethod):
    ...
    def optimize(
        ...
        constraint_results = self.constraints.check_all(
            results.final_dose_grid, structures
        )
        all_satisfied = all(
            info["is_satisfied"] for info in constraint_results.values()
        )
        ...
        if all_satisfied:
            ...
            unsatisfied = {
                name: info
                for name, info in constraint_results.items()
                if not info["is_satisfied"]
            }
            logger.warning(f"Có {len(unsatisfied)} ràng buộc không được thỏa mãn")
            for name, info in unsatisfied.items():
                logger.warning(
                    f"- {name}: {info['description']}, vi phạm: {info['violation']:.4f}"
                )
...
class PenaltyMethod(ConstraintBasedMethod):
    ...
    def optimize(
        ...
        penalty_objective = ConstraintViolationObjective(
            constraints=self.constraints, weight=current_penalty_weight
        )
        ...
        for i in range(max_iterations):
            logger.info(
                f"Vòng lặp hàm phạt {i + 1}/{max_iterations}, trọng số phạt: {current_penalty_weight}"
            )
            ...
            final_dose_grid, results = super().optimize(
                current_dose_grid, structures, solver_name
            )
            ...
            if all_satisfied:
                logger.info(
                    "Tất cả các ràng buộc đều được thỏa mãn, dừng vòng lặp hàm phạt"
                )
                break
...
class AugmentedLagrangianMethod(ConstraintBasedMethod):
    ...
    def optimize(
        ...
        all_results = []
        previous_max_violation = float(
            "inf"
        )  # Khởi tạo giá trị ban đầu cho vi phạm lớn nhất
        ...
        for i in range(max_outer_iterations):
            logger.info(
                f"Vòng lặp Lagrangian tăng cường {i + 1}/{max_outer_iterations}, hệ số phạt: {current_penalty}"
            )
            ...
            constraint_violations = {
                i: info["violation"]
                for i, (_, info) in enumerate(constraint_results.items())
            }

            # Kiểm tra điều kiện dừng
            max_violation = (
                max(constraint_violations.values()) if constraint_violations else 0
            )
            if max_violation < self.tolerance:
                logger.info(
                    f"Đạt đến độ chính xác yêu cầu (vi phạm lớn nhất: {max_violation:.6f}), dừng vòng lặp"
                )
                break

            # Cập nhật nhân tử Lagrange
            for i, violation in constraint_violations.items():
                self.multipliers[i] = max(
                    0, self.multipliers[i] + current_penalty * violation
                )

            # Tăng hệ số phạt nếu vi phạm chưa giảm đủ nhanh
            if i > 0 and max_violation > 0.25 * previous_max_violation:
                current_penalty = min(
                    current_penalty * self.penalty_increase_factor, self.max_penalty
                )
...
class ConstraintViolationObjective(ObjectiveFunction):
    ...
    def __init__(
        ...
        """
        # Create a dummy structure ID for compatibility
        structure_id = "constraint_violation"
        super().__init__(
            structure_id=structure_id, structure_name=structure_name, weight=weight
        )
        self.constraints = constraints
    ...
    def evaluate(self, dose, structure_mask) -> float:
        ...
        if not engine or not hasattr(engine, "structures"):
            logger.warning(
                "Không thể truy cập vào cấu trúc từ engine, không thể đánh giá vi phạm ràng buộc"
            )
            return 0.0

        # Kiểm tra tất cả các ràng buộc
        constraint_results = self.constraints.check_all(
            dose if isinstance(dose, DoseGrid) else None, engine.structures
        )

        # Tính tổng bình phương của vi phạm
        total_violation = sum(
            info["violation"] ** 2 for info in constraint_results.values()
        )
    ...
    def get_gradient(self, dose, structure_mask) -> np.ndarray:
        """
        Tính toán gradient của hàm vi phạm ràng buộc.
        ...
        """
        # Đây là phiên bản gradient đơn giản
        # Trong thực tế, cần phải tính gradient chính xác cho mỗi loại ràng buộc

        # Mô phỏng gradient xấp xỉ bằng sai phân hữu hạn
        epsilon = 1e-5
        gradient = np.zeros_like(dose)

        # Không thể tính gradient chính xác cho tất cả các ràng buộc mà không có thông tin chi tiết
        # Trả về gradient zero là một xấp xỉ an toàn nhưng không tối ưu
        logger.warning(
            "get_gradient cho ConstraintViolationObjective trả về gradient zero - cần triển khai chi tiết hơn"
        )
...
class AugmentedLagrangianObjective(ObjectiveFunction):
    ...
    def __init__(
        ...
        """
        # Create a dummy structure ID for compatibility
        structure_id = "augmented_lagrangian"
        super().__init__(
            structure_id=structure_id, structure_name=structure_name, weight=1.0
        )
        self.constraints = constraints
    ...
    def evaluate(self, dose, structure_mask) -> float:
        ...
        if not engine or not hasattr(engine, "structures"):
            logger.warning(
                "Không thể truy cập vào cấu trúc từ engine, không thể đánh giá Lagrangian"
            )
            return 0.0
        ...
        for obj in engine.objectives:
            if obj != self:  # Tránh đệ quy vô hạn
                original_objective += obj.evaluate(dose, engine.structures)

        # Kiểm tra tất cả các ràng buộc
        constraint_results = {}
        for i, constraint in enumerate(self.constraints):
            is_satisfied, violation = constraint.check(dose, engine.structures)
        ...
        for i, violation in constraint_results.items():
            ...
            lagrangian_term += multiplier * violation + (self.penalty / 2) * (
                violation**2
            )
    ...
    def get_gradient(self, dose, structure_mask) -> np.ndarray:
        """
        Tính toán gradient của hàm Lagrangian tăng cường.
        ...
        """
        # Đây là phiên bản gradient đơn giản
        # Trong thực tế, cần phải tính gradient chính xác

        # Mô phỏng gradient xấp xỉ bằng sai phân hữu hạn
        gradient = np.zeros_like(dose)

        # Không thể tính gradient chính xác cho Lagrangian tăng cường mà không có thông tin chi tiết
        # Trả về gradient zero là một xấp xỉ an toàn nhưng không tối ưu
        logger.warning(
            "get_gradient cho AugmentedLagrangianObjective trả về gradient zero - cần triển khai chi tiết hơn"
        )
...
def create_constraint_based_method(
    ...
    **kwargs,
    ...
    if method_type == "constraint_satisfaction":
        ...
        return ConstraintSatisfactionMethod(
            constraints, objectives, parameters, feasibility_threshold
        )
        ...
        return PenaltyMethod(
            objectives, constraints, parameters, penalty_weight, adaptive_weights
        )
        ...
        return AugmentedLagrangianMethod(
            objectives, constraints, parameters, initial_penalty, initial_multipliers
        )
    else:
