...
class KBPFeatureExtractor:
    ...
    def extract_features(
        ...
        geometry_features = get_geometry_features(
        ...
        features = KBPFeatures(
            ...
            # Khoảng cách từ PTV đến các OAR
            distances_to_oars={
                oar: geometry_features[f"{ptv}_to_{oar}_distance"] for oar in oars
            },
            # Thể tích trùng lặp
            overlap_volumes={
                oar: geometry_features[f"{ptv}_to_{oar}_overlap"] for oar in oars
            },
            # Tỷ lệ trùng lặp
            overlap_ratios={
                oar: geometry_features[f"{ptv}_to_{oar}_overlap_ratio"] for oar in oars
            },
            # Đặc trưng lâm sàng
...
class KBPModel:
    ...
    def __init__(
        ...
        self.model_metrics = {}  # Dict chứa các metrics đánh giá mô hình
        self.training_stats = {
            "num_samples": 0,
            "training_date": None,
            "site": "",
    ...
    def _create_model(self):
        ...
        if self.model_type == ModelType.RANDOM_FOREST:
            ...
            try:
                # Thử tạo mô hình LightGBM nếu có sẵn
                import lightgbm as lgb

                return lgb.LGBMRegressor(**self.model_params)
            except ImportError:
                logger.warning(
                    "LightGBM không có sẵn. Sử dụng GradientBoosting thay thế."
                )
                return GradientBoostingRegressor(**self.model_params)
    ...
    def evaluate_model(
        ...
        """
        Đánh giá chi tiết mô hình trên tập kiểm tra.

        Args:
            X_test: Features của tập kiểm tra
            y_test: Target values của tập kiểm tra

        Returns:
            Dict chứa các metrics đánh giá cho từng mục tiêu
        """
        evaluation_results = {}

        for target_name, y_true in y_test.items():
            if target_name not in self.models:
                logger.warning(f"Không có mô hình cho mục tiêu {target_name}")
                continue
            ...
            # Thêm percentile errors
            for p in [90, 95, 99]:
                metrics[f"percentile_{p}_error"] = np.percentile(
                    np.abs(y_true - y_pred), p
                )

            evaluation_results[target_name] = metrics

            # Lưu lại kết quả đánh giá
    ...
    def get_feature_importances(self, target_name: str) -> Dict[str, float]:
        """
        Lấy tầm quan trọng của các đặc trưng cho một mục tiêu cụ thể.

        Args:
            target_name: Tên mục tiêu

        Returns:
            Dict mapping tên đặc trưng tới mức độ quan trọng
        """
        if target_name not in self.models:
            raise KeyError(f"Không có mô hình cho mục tiêu {target_name}")

        model = self.models[target_name]

        # Lấy tầm quan trọng của đặc trưng từ mô hình nếu có
        if hasattr(model, "feature_importances_"):
            ...
            logger.warning(
                f"Mô hình cho {target_name} không hỗ trợ tính năng trích xuất tầm quan trọng của đặc trưng"
            )
            return {}

        # Map tầm quan trọng tới tên đặc trưng
        feature_names = self.training_stats.get("features_used", [])
        ...
        # Tạo và sắp xếp theo tầm quan trọng giảm dần
        importance_dict = {feature_names[i]: float(importances[i]) for i in range(n)}
        ...
        # Chỉ lưu top 10 đặc trưng quan trọng nhất
        top_features = dict(list(sorted_importances.items())[:10])
        self.feature_importances[target_name] = top_features

        return top_features

    def get_model_summary(self) -> Dict[str, Any]:
        """
        Tạo tóm tắt thông tin về mô hình.

        Returns:
            Dict chứa thông tin tóm tắt về mô hình
        """
        # Tính độ chính xác trung bình trên tất cả các mục tiêu
        avg_r2 = 0.0
        count = 0
        ...
        # Tổng hợp top đặc trưng quan trọng từ tất cả mục tiêu
        all_importances = {}

        for target_name, importances in self.feature_importances.items():
        ...
        if all_importances:
            ...
            all_importances = {
                k: v
                for k, v in sorted(
                    all_importances.items(), key=lambda item: item[1], reverse=True
                )
            }

            # Chỉ lấy top 10
            all_importances = dict(list(all_importances.items())[:10])

        return {
            "model_name": self.model_name,
            "model_type": str(self.model_type),
            "site": self.training_stats.get("site", ""),
            "num_samples": self.training_stats.get("num_samples", 0),
            "training_date": self.training_stats.get("training_date"),
            "average_r2": avg_r2,
            "targets": list(self.models.keys()),
            "important_features": all_importances,
            "num_features": len(self.training_stats.get("features_used", [])),
    ...
    def train(
        ...
        for target_name, y_values in y_train.items():
            ...
            metrics[target_name] = {"mse": mse, "rmse": np.sqrt(mse), "r2": r2}
            ...
            if hasattr(best_model, "feature_importances_"):
                self.feature_importances[target_name] = {
                    ...
                    for feature, importance in zip(
                        X_train.columns, best_model.feature_importances_
                    )
                }

        return metrics

    def predict(self, features: Union[KBPFeatures, pd.DataFrame]) -> KBPPredictions:
        """
        ...
        oar_dose_constraints = {
            k: v for k, v in predictions.items() if k.startswith("dose_constraint_")
        }

        # Dự đoán trọng số mục tiêu
        objective_weights = {
            k: v for k, v in predictions.items() if k.startswith("weight_")
        }
        ...
        for k, v in predictions.items():
            if k.startswith("objective_"):
                ...
                structure_objectives[struct_name].append({"type": obj_type, "value": v})
    ...
    def save(self, directory: str) -> None:
        ...
        for target_name, model in self.models.items():
            model_path = os.path.join(
                directory, f"{self.model_name}_{target_name}.joblib"
            )
        ...
        for target_name, scaler in self.scalers.items():
            scaler_path = os.path.join(
                directory, f"{self.model_name}_{target_name}_scaler.joblib"
            )
    ...
    def load(cls, directory: str, model_name: str) -> "KBPModel":
        ...
        for target_name in metadata["targets"]:
            ...
            scaler_path = os.path.join(
                directory, f"{model_name}_{target_name}_scaler.joblib"
            )
