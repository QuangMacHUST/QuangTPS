...
class ReportGenerator:
    ...
    def __init__(self, output_dir: Optional[str] = None):
        ...
        self.jinja_env.filters["format_date"] = (
            lambda d: d.strftime("%d/%m/%Y")
            if isinstance(d, datetime.datetime)
            else str(d)
        )
        self.jinja_env.filters["format_number"] = (
        ...
        try:
            ...
            if output_path is None:
                ...
                output_path = os.path.join(
                    self.output_dir, f"plan_report_{plan_id}_{timestamp}.pdf"
                )
    ...
    def generate_patient_report(
        ...
        try:
            ...
            if output_path is None:
                ...
                output_path = os.path.join(
                    self.output_dir, f"patient_report_{patient_id}_{timestamp}.pdf"
                )
    ...
    def generate_template_report(
        ...
        try:
            ...
            if output_path is None:
                timestamp = datetime.datetime.now().strftime("%Y%m%d_%H%M%S")
                output_path = os.path.join(
                    self.output_dir, f"report_{template_name}_{timestamp}.pdf"
                )
    ...
    def _generate_html_report(self, template_path, data, output_path):
        ...
        try:
            ...
            try:
                ...
                logger.warning(
                    "WeasyPrint not available. Trying alternative PDF generation methods..."
                )
                ...
                try:
                    ...
                    logger.warning(
                        "pdfkit not available either. Using matplotlib as fallback."
                    )
                    HAS_PDFKIT = False
            ...
            if HAS_WEASYPRINT:
                ...
                try:
                    ...
                    logger.error(
                        f"Fallback PDF generation failed: {str(fallback_error)}"
                    )
    ...
    def _convert_html_to_pdf_fallback(self, html, output_path):
        ...
        with PdfPages(output_path) as pdf:
            ...
            ax.text(
                0.1,
                0.9,
                "HTML Rendering Not Available - Basic Content:",
                fontweight="bold",
            )
            ...
            ax.text(
                0.5,
                0.05,
                f"Note: This is a fallback PDF as WeasyPrint was not available.",
                ha="center",
                fontsize=8,
                style="italic",
            )
    ...
    def _generate_plan_summary(self, pdf, plan, include_patient=True):
        ...
        if hasattr(plan, "prescription") and plan.prescription:
            ...
            ax.text(
                0.3, y_pos, f"{getattr(prescription, 'dose_per_fraction', 'N/A')} Gy"
            )
        else:
            y_pos -= 0.05
        ...
        ax.text(
            0.5, 0.03, "QuangTPS Treatment Planning System", ha="center", fontsize=9
        )
    ...
    def _generate_patient_summary(self, pdf, patient):
        ...
        # Title
        ax.text(
            0.5, 0.95, "Patient Summary", fontsize=20, ha="center", fontweight="bold"
        )
    ...
    def _generate_dvh_page(self, pdf, plan):
        ...
        try:
            ...
            has_structures = (
                hasattr(plan, "structure_set") and plan.structure_set is not None
            )
            ...
            if has_dose and has_structures:
                ...
                if isinstance(plan.dose_grid, np.ndarray):
                    dose_grid = plan.dose_grid
                elif hasattr(plan.dose_grid, "data") and isinstance(
                    plan.dose_grid.data, np.ndarray
                ):
                ...
                if dose_grid is not None and structures:
                    ...
                    ax.text(
                        0.5,
                        0.5,
                        "No dose or structure data available",
                        ha="center",
                        va="center",
                        fontsize=12,
                    )
    ...
    def _generate_metrics_page(self, pdf, plan):
        ...
        if has_metrics:
            ...
            for target, coverage in plan.evaluation_metrics.get(
                "target_coverage", {}
            ).items():
                y_pos -= 0.05
            ...
            y_pos -= 0.1
            ax.text(
                0.1, y_pos, "Organ-at-Risk Constraints", fontsize=14, fontweight="bold"
            )

            for oar, constraints in plan.evaluation_metrics.get(
                "oar_constraints", {}
            ).items():
                y_pos -= 0.05
    ...
    def _generate_beam_parameters(self, pdf, plan):
        ...
        # Title
        ax.text(
            0.5, 0.95, "Beam Parameters", fontsize=20, ha="center", fontweight="bold"
        )
