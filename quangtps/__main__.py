...
def parse_arguments():
    ...
    parser = argparse.ArgumentParser(
        description="QuangTPS - Hệ thống Lập kế hoạch Xạ trị Mã nguồn Mở"
    )
    ...
    parser.add_argument(
        "--batch", type=str, help="Chạy chế độ xử lý hàng loạt với file cấu hình"
    )
    parser.add_argument(
        "--check-system",
        action="store_true",
        help="Kiểm tra hệ thống và hiển thị thông tin",
    )
    parser.add_argument("--version", action="store_true", help="Hiển thị phiên bản")
    parser.add_argument(
        "--setup", action="store_true", help="Thiết lập và chuẩn bị môi trường"
    )
...
def show_splash_screen(app):
    ...
    if not os.path.exists(splash_path):
        ...
        painter.drawText(
            pixmap.rect().adjusted(0, 50, 0, 0), Qt.AlignHCenter, "QuangTPS"
        )
        ...
        painter.drawText(
            pixmap.rect().adjusted(0, 180, 0, 0), Qt.AlignHCenter, "Phiên bản 1.0.0"
        )
...
def main():
    ...
    if args.batch:
        ...
        if logger:
            logger.info(
                "Chạy chế độ xử lý hàng loạt với cấu hình: %s", batch_config_path
            )
    ...
    try:
        # Ensure required components are imported
        try:
            from quangtps.ui.dvh_view import DVHView

            if logger:
                logger.info("DVH View module loaded successfully")
        except ImportError as e:
            if logger:
                logger.warning(f"Could not load DVH View module: {str(e)}")
        ...
        # Create a timer to show the main window after splash screen
...
def setup_application(app):
    ...
    icon_path = os.path.join(
        os.path.dirname(os.path.abspath(__file__)), "ui", "icons", "logo.png"
    )
