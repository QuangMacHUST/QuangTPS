...
from PyQt5.QtWidgets import (
    ...
    QComboBox, QSlider, QGroupBox, QTabWidget, QSplitter,
    QFrame, QGridLayout, QCheckBox, QToolButton, QSpinBox,
    QDoubleSpinBox, QRadioButton, QButtonGroup, QTableWidget,
...
class ImageRegistrationPanel(QWidget):
    ...
    def _setup_ui(self):
        ...
        self.metric_combo.addItems(["Mutual Information", "Normalized Cross Correlation", "Mean Squares"])
        ...
        self.roi_reg_check.setToolTip("Limit registration to a specific region of interest")
        ...
        # Instructions
        point_layout.addWidget(QLabel("Create corresponding points in both image sets:"))
        
        # Points table
        self.points_table = QTableWidget(0, 3)
        self.points_table.setHorizontalHeaderLabels(["Point Name", "Primary (mm)", "Secondary (mm)"])
    ...
    def set_images(self, primary_image: DicomSeries, secondary_images: Dict[str, DicomSeries]):
        """
    ...
    def _update_image_selectors(self):
        ...
        if self.primary_image:
            self.primary_combo.addItem(self.primary_image.series_description or "Primary Image", self.primary_image)
    ...
    def _update_roi_selector(self):
        ...
        if hasattr(self, 'parent') and self.parent and hasattr(self.parent, 'current_patient'):
    ...
    def _adjust_transform(self, param: str, delta: float):
        ...
        step = 1.0 if param.startswith("trans") else 1.0  # 1mm for translation, 1deg for rotation
    ...
    def _run_auto_registration(self):
        ...
        try:
            ...
            logger.info(f"Running auto registration with resolution level {resolution_level}, "
    ...
    def _run_point_registration(self):
        ...
        if len(points_primary) < 3:
            logger.warning("At least 3 valid points are needed for point-based registration")
            return
        ...
        try:
            ...
            logger.info(f"Running point-based registration with {len(primary_array)} points")
    ...
    def _apply_registration(self):
        ...
        self.registration_completed.emit(
            self.primary_image.series_uid,
            self.current_secondary.series_uid,
            transform
        )
    ...
    def _save_registration(self):
        ...
        logger.info("Registration save functionality will be implemented in a future update")
