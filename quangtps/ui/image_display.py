...
# Try to import matplotlib for advanced visualization
try:
    import matplotlib
    ...
    logging.warning("Matplotlib not available. Advanced visualization will be limited.")
...
class ImageSliceWidget(QWidget):
    ...
    mouse_released = pyqtSignal(
        int, int, int, Qt.MouseButton
    )  # slice_idx, x, y, button
    ...
    def __init__(self, parent=None):
        ...
        self.contours = {}  # {name: {slice_idx: points}}
        self.contour_colors = {}  # {name: QColor}
        self.contour_visibility = {}  # {name: bool}
        self.dose_data = None
        ...
        self.window_min = 0
        self.window_max = 100
        self.view_plane = "axial"  # axial, sagittal, coronal
        self.brightness = 0
        self.contrast = 1.0
    ...
    def add_contour(self, name, contour_points, slice_idx=None, color=None):
        ...
        for i, (idx, points) in enumerate(self.contours[name].items()):
            if idx == slice_idx:
                self.contours[name][idx] = contour_points
                contour_updated = True
                break

        if not contour_updated:
            self.contours[name][slice_idx] = contour_points
    ...
    def remove_contour(self, name, slice_idx=None):
        ...
        if slice_idx is None:
            ...
            if slice_idx in self.contours[name]:
    ...
    def _update_display(self):
        ...
        window_min, window_max = self.window_min, self.window_max
        normalized_data = np.clip(slice_data, window_min, window_max)
        normalized_data = (
            (normalized_data - window_min) / (window_max - window_min) * 255
        ).astype(np.uint8)
        ...
        if self.contours:
            ...
            for name, contour_dict in self.contours.items():
                if not self.contour_visibility.get(name, True):
                    continue

                for idx, points in contour_dict.items():
                    if idx == self.slice_idx:
    ...
    def set_brightness(self, value):
        ...
        self.set_hounsfield_window(
            new_center - window_width // 2, new_center + window_width // 2
        )
    ...
    def set_contrast(self, value):
        ...
        # Thiết lập cửa sổ Hounsfield mới
        self.set_hounsfield_window(
            window_center - new_width // 2, window_center + new_width // 2
        )
...
class ImageControlWidget(QWidget):
    ...
    def __init__(self, parent=None):
        """Khởi tạo ImageControlWidget."""
        super().__init__(parent)

        # Khởi tạo giá trị mặc định
        self.current_plane = "axial"  # axial, sagittal, coronal
        self.current_slice = 50
        self.max_slice = 100
        self.window_min = 0
        self.window_max = 1000
...
class ImageDisplay(QWidget):
    ...
    def __init__(self, parent=None):
        ...
        self.handle_wheel_event = (
            False  # Mặc định không xử lý sự kiện wheel, để xử lý ở cấp cao hơn
        )
    ...
    def _update_display(self):
        ...
        if self.image_data is None:
            ...
            if self.title:
                try:
                    ...
                    painter.drawText(
                        pixmap.rect(), Qt.AlignmentFlag.AlignCenter, self.title
                    )
                    painter.end()
        ...
        try:
            ...
            if not isinstance(self.image_data, np.ndarray):
                logger.error(
                    f"Dữ liệu hình ảnh không phải là mảng NumPy: {type(self.image_data)}"
                )
                if self.title:
            ...
            if self.image_data.size == 0:
                ...
                if self.title:
                    ...
                    painter.setFont(font)
                    error_message = (
                        f"{self.title if self.title else 'Error'}\nLỗi: Dữ liệu rỗng"
                    )
                    painter.drawText(pixmap.rect(), Qt.AlignCenter, error_message)
                    painter.end()
                    self.image_frame.setPixmap(pixmap)
            ...
            if self.image_data.ndim < 2:
                logger.error(
                    f"Dữ liệu hình ảnh cần ít nhất 2 chiều, nhưng có {self.image_data.ndim} chiều"
                )
                # Hiển thị thông báo lỗi tương tự
                if self.title:
                    blank_image = np.zeros((200, 200), dtype=np.uint8)
            ...
            logger.debug(
                f"Đang hiển thị hình ảnh: shape={self.image_data.shape}, "
            ...
            if min_val >= max_val:
                logger.warning(
                    f"Window không hợp lệ: min={min_val}, max={max_val}, điều chỉnh để có khoảng cách tối thiểu"
                )
            ...
            try:
                ...
                if np.issubdtype(self.image_data.dtype, np.integer) and (
                    min_val < np.iinfo(self.image_data.dtype).min
                    or max_val > np.iinfo(self.image_data.dtype).max
                ):
                    logger.debug(
                        f"Chuyển đổi dữ liệu từ {self.image_data.dtype} sang float32 để tránh tràn số"
                    )
                ...
                try:
                    ...
                    if self.title:
                        ...
                        h, w = blank_image.shape
                        qimage = QImage(
                            blank_image.data, w, h, w, QImage.Format_Grayscale8
                        )
                        pixmap = QPixmap.fromImage(qimage)
                        painter = QPainter(pixmap)
                        painter.setPen(QPen(Qt.red))
            ...
            try:
                if max_val > min_val:
                    normalized = (
                        (clipped - min_val) / (max_val - min_val) * 255
                    ).astype(np.uint8)
                else:
                ...
                try:
                    ...
                    if np.isfinite(clipped).all():
                        ...
                        if range_val > 0:
                            normalized = np.round(
                                ((clipped_finite - min_val) / range_val) * 255
                            ).astype(np.uint8)
                        else:
            ...
            try:
                ...
                if pixmap.width() <= 0 or pixmap.height() <= 0:
                    raise ValueError(
                        f"QPixmap có kích thước không hợp lệ: {pixmap.width()}x{pixmap.height()}"
                    )
                ...
                try:
                    ...
                    bytes_per_line = w
                    qimage = QImage(
                        data_copy.data, w, h, bytes_per_line, QImage.Format_Grayscale8
                    )
                    ...
                    logger.info(
                        "Đã tạo thành công QImage/QPixmap bằng phương pháp thay thế"
                    )
                except Exception as e2:
                    ...
                    if self.title:
                        ...
                        painter.setPen(QPen(Qt.red))
                        painter.drawText(
                            pixmap.rect(), Qt.AlignCenter, f"{self.title}\nLỗi hiển thị"
                        )
                        painter.end()
                        self.image_frame.setPixmap(pixmap)
                    return
    ...
    def mouseMoveEvent(self, event):
        ...
        self.mouse_position_changed.emit(
            img_x, img_y, self._get_pixel_value(img_x, img_y)
        )
    ...
    def _screen_to_image_coords(self, x, y):
        ...
        if self.image_data is not None:
            img_width = (
                self.image_data.shape[1] if len(self.image_data.shape) > 1 else 1
            )
            img_height = (
                self.image_data.shape[0] if len(self.image_data.shape) > 0 else 1
            )
    ...
    def _get_pixel_value(self, x, y):
        ...
        if (
            ...
            and 0 <= y < self.image_data.shape[0]
            and 0 <= x < self.image_data.shape[1]
        ):
