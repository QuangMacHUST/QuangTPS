...
try:
    ...
    from quangtps.evaluation.clinical_protocols import (
        ClinicalProtocol,
        ClinicalProtocolManager as ProtocolManager,
    )
except ImportError:
...
class ProtocolEditorDialog(QDialog):
    ...
    def edit_goal(self, row=None):
        ...
        if (
            ...
            or row >= len(self.protocol.goals)
        ):
            return
    ...
    def save_protocol(self):
        ...
        try:
            ...
            QMessageBox.critical(
                self,
                "Save Error",
                f"An error occurred while saving the protocol: {str(e)}",
            )
...
if __name__ == "__main__":
    ...
    class SampleProtocolManager:
        def save_protocol(self, protocol):
            ...
            for goal in protocol.goals:
                print(
                    f"  {goal.structure_name}: {goal.goal_type} {goal.operator} {goal.value}"
                )
            return True
    ...
    dialog = ProtocolEditorDialog(
        protocol=protocol, protocol_manager=SampleProtocolManager()
    )
