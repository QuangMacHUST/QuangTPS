...
from PyQt5.QtWidgets import (
    ...
    QInputDialog, QStackedWidget, QAbstractItemView, QSizePolicy
)
...
try:
    ...
    from quangtps.segmentation.manual_segmentation.brush_tool import BrushTool, BrushToolWidget
    from quangtps.segmentation.manual_segmentation.threshold_tool import ThresholdContourTool, ThresholdToolWidget, ThresholdMode, ThresholdOperation
...
class ObjectExplorerPanel(QWidget):
    ...
    def init_ui(self):
        ...
        header.setStyleSheet("font-weight: bold; background-color: #0078D7; color: white; padding: 5px;")
    ...
    def update_displayed_objects(self):
        ...
        try:
            ...
            if patient_db:
                ...
                if images:
                    for image in images:
                        image_item = self._create_item(
                            image.description if hasattr(image, 'description') else "Image",
                ...
                structure_sets = patient_db.get_structure_sets_for_patient(self.current_patient.id)
                if structure_sets:
    ...
    def _create_item(self, title: str, subtitle: str, is_selected: bool = False,
                    item_type: str = "", item_data: Any = None) -> QWidget:
        ...
        item.setStyleSheet("background-color: #f0f0f0; border: 1px solid #ddd; border-radius: 3px;")
        if is_selected:
            item.setStyleSheet("background-color: #0078D7; color: white; border: 1px solid #0078D7; border-radius: 3px;")
...
class StructureTab(QWidget):
    ...
    def create_structure_sidebar(self):
        ...
        self.structure_list.customContextMenuRequested.connect(self.show_structure_context_menu)
        ...
        self.add_structure_btn.setIcon(QIcon(os.path.join(os.path.dirname(__file__), "icons", "add.png")))
        ...
        self.delete_structure_btn.setIcon(QIcon(os.path.join(os.path.dirname(__file__), "icons", "delete.png")))
        ...
        self.edit_structure_btn.setIcon(QIcon(os.path.join(os.path.dirname(__file__), "icons", "edit.png")))
    ...
    def create_content_area(self):
        ...
        self.segmentation_interface.structureModified.connect(self.on_structure_modified)
        ...
        auto_segment_action.setIcon(QIcon(os.path.join(os.path.dirname(__file__), "icons", "auto_segment.png")))
        ...
        smart_brush_action.setIcon(QIcon(os.path.join(os.path.dirname(__file__), "icons", "smart_brush.png")))
        smart_brush_action.triggered.connect(lambda: self.segmentation_interface.set_current_tool("smart_brush"))
        ...
        copy_slices_action.setIcon(QIcon(os.path.join(os.path.dirname(__file__), "icons", "copy_slices.png")))
    ...
    def setup_connections(self):
        """Set up connections between widgets."""
        self.structure_list.currentItemChanged.connect(self.on_structure_selection_changed)
    ...
    def update_property_display(self):
        ...
        if self.selected_structure:
            ...
            if self.image:
                voxel_size = self.image.voxel_size if hasattr(self.image, 'voxel_size') else (1.0, 1.0, 1.0)
    ...
    def edit_structure_properties(self):
        ...
        name, ok = QInputDialog.getText(
            ...
            text=self.selected_structure.name
        )
        ...
        if color_dialog.exec_():
            qcolor = color_dialog.currentColor()
            self.selected_structure.color = (qcolor.red(), qcolor.green(), qcolor.blue())
    ...
    def handle_mouse_event(self, event_type, point, slice_index=None, orientation=None):
        ...
        if event_type == "press":
            return self.segmentation_interface.handle_mouse_press(point, slice_index, orientation)
    ...
    def get_overlay_for_viewer(self, orientation, slice_index):
        ...
        return self.segmentation_interface.get_overlay_for_viewer(orientation, slice_index)
...
def test_structure_tab():
    ...
    class TestPatientDB:
        ...
        def get_structure_sets_for_patient(self, patient_id):
            return [ss for ss in self.structure_sets.values() if ss.patient_id == patient_id]
