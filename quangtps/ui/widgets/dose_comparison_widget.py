...
class DoseComparisonWidget(QWidget):
    ...
    def _init_ui(self):
        ...
        self.dose_threshold_spinner.valueChanged.connect(
            self._on_gamma_criteria_changed
        )
        ...
        self.distance_threshold_spinner.setValue(
            self.gamma_criteria["distance_threshold"]
        )
        self.distance_threshold_spinner.setSuffix(" mm")
        self.distance_threshold_spinner.valueChanged.connect(
            self._on_gamma_criteria_changed
        )
    ...
    def _on_gamma_criteria_changed(self):
        ...
        self.gamma_criteria["distance_threshold"] = (
            self.distance_threshold_spinner.value()
        )
        self.gamma_criteria["local_normalization"] = (
            self.local_norm_checkbox.isChecked()
        )
        ...
        if self.display_mode == "subtraction":
            if not hasattr(self, "dose_diff"):
                ax.text(
                    0.5,
                    0.5,
                    "No dose difference data available",
                    ha="center",
                    va="center",
