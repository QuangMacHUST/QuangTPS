...
# Tương thích với cả hai loại interface
try:
...
class SliderWithLabel(QWidget):
    ...
    def set_value(self, value: float):
        normalized_value = int(
            (value - self.min_val) / (self.max_val - self.min_val) * 100
        )
        self.slider.setValue(normalized_value)

    def get_value(self) -> float:
        return self.min_val + (self.slider.value() / 100.0) * (
...
class TradingWidget(QWidget):
    ...
    def set_objectives(self, objectives: Dict[str, Objective]):
        ...
        for name, objective in sorted(
            objectives.items(), key=lambda x: 0 if "PTV" in x[0] else 1
        ):
...
class ParetoPlotWidget(QWidget):
    ...
    def _update_plot(self):
        ...
        for solution in self.solutions:
            if (
                x_name in solution.objective_values
                and y_name in solution.objective_values
            ):
        ...
        if self.selected_index is not None and 0 <= self.selected_index < len(
            ...
            if (
                x_name in selected_solution.objective_values
                and y_name in selected_solution.objective_values
            ):
...
class ObjectiveSlider(QWidget):
    ...
    def __init__(
        self, objective_name: str, min_val: float, max_val: float, parent=None
    ):
        super().__init__(parent)
...
class WeightSlider(QWidget):
    ...
    def _setup_ui(self):
        ...
        self.name_label = QLabel(
            f"Plan {self.solution_index + 1}: {self.solution_name}"
        )
        self.name_label.setMinimumWidth(150)
        layout.addWidget(self.name_label)
...
class MCOPanel(QWidget):
    ...
    def __init__(self, parent=None):
        super().__init__(parent)
        self.plan = None
        self.mco_engine = None
        self.pareto_navigator = None  # Thêm hỗ trợ ParetoNavigator mới
    ...
    def set_plan(self, plan: Plan):
        ...
        self.mco_engine = MCONavigator(plan)
    ...
    def _on_generate_clicked(self):
        ...
        if not self.mco_engine or not self.plan:
        ...
        if optimization_service:
            objectives = optimization_service.get_objectives_for_plan(self.plan)
            ...
            self.mco_engine.set_objectives(objectives)
    ...
    def _start_generation(self, num_plans: int):
        ...
        # Start generation
        if self.mco_engine:
            success = self.mco_engine.generate_pareto_plans(num_plans)
            ...
            if success:
                ...
                QMessageBox.critical(
                    self, "Error", "Failed to generate Pareto-optimal plans"
                )
                self.progress_bar.setVisible(False)
                self.generate_button.setEnabled(True)
    ...
    def _create_objective_sliders(self):
        ...
        for obj_name in self.mco_engine.pareto_surface.objective_names:
            # Get range of this objective
            min_val, max_val = self.mco_engine.get_objective_range(obj_name)
            ...
            self.objective_sliders[obj_name] = slider
    ...
    def _on_objective_slider_changed(self, objective_name: str, value: float):
        ...
        Args:
            ...
            value: New value
        """
        if self.is_normalizing or not self.mco_engine:
            return
        ...
        closest_idx = self.mco_engine.pareto_surface.find_closest_solution(
            objective_values
        )
    ...
    def _on_weight_slider_changed(self, solution_index: int, value: float):
        ...
        if weights:
            ...
            if weight_sum > 0:
                ...
                # Interpolate plan
                interpolated_plan = self.mco_engine.interpolate(normalized_weights)
    ...
    def _normalize_weights(self, changed_index: int):
        ...
        if weight_sum > 1.0:
            ...
            if weights[changed_index] >= 1.0:
                ...
                other_weights_sum = sum(
                    w for i, w in weights.items() if i != changed_index
                )
                    ...
                    for slider in self.weight_sliders.values():
                        ...
                        if idx != changed_index:
                            new_weight = max(
                                0,
                                weights[idx]
                                - (excess * weights[idx] / other_weights_sum),
                            )
    ...
    def _on_normalize_changed(self, checked: bool):
        ...
        if checked:
            # Normalize weights
            weight_sum = sum(
                slider.get_value() for slider in self.weight_sliders.values()
            )
    ...
    def _update_objective_sliders(self):
        ...
        if not self.mco_engine or not self.mco_engine.current_solution:
            return
        ...
        for slider in self.objective_sliders.values():
            ...
            value = self.mco_engine.current_solution.get_objective_value(obj_name)
            slider.set_value(value)

        self.is_normalizing = False

    def _update_visualization(self):
        """Cập nhật hiển thị hình ảnh visualization."""
        try:
            if self.pareto_navigator:
                # Ưu tiên dùng ParetoNavigator nếu có
                if hasattr(self.pareto_chart, "clear"):
                    self.pareto_chart.clear()

                solutions = []
                if hasattr(self.pareto_navigator, "get_all_solutions"):
                    solutions = self.pareto_navigator.get_all_solutions()
                elif hasattr(self.pareto_navigator, "pareto_surface") and hasattr(
                    self.pareto_navigator.pareto_surface, "solutions"
                ):
                    solutions = self.pareto_navigator.pareto_surface.solutions

                # Nếu có giải pháp, hiển thị lên đồ thị
                if solutions and hasattr(self.pareto_chart, "set_solutions"):
                    self.pareto_chart.set_solutions(solutions)

                    # Đánh dấu giải pháp hiện tại
                    if self.current_solution and hasattr(
                        self.pareto_chart, "highlight_solution"
                    ):
                        solution_id = getattr(self.current_solution, "id", None)
                        if solution_id:
                            self.pareto_chart.highlight_solution(solution_id)

                # Cập nhật đồ thị radar nếu có
                if hasattr(self, "radar_chart") and self.current_solution:
                    try:
                        self.radar_chart.update_solution(self.current_solution)
                    except Exception as e:
                        logger.error(f"Lỗi cập nhật radar chart: {str(e)}")

                # Cập nhật DVH nếu có
                if (
                    hasattr(self, "dvh_widget")
                    and self.current_solution
                    and hasattr(self.current_solution, "dvh_data")
                ):
                    try:
                        self.dvh_widget.set_dvh_data(self.current_solution.dvh_data)
                    except Exception as e:
                        logger.error(f"Lỗi cập nhật DVH: {str(e)}")

            # Fallback to old method if needed
            elif self.mco_engine and hasattr(self.pareto_chart, "set_solutions"):
                solutions = self.mco_engine.get_solutions()
                if solutions:
                    self.pareto_chart.set_solutions(solutions)
                    if self.current_solution:
                        self.pareto_chart.highlight_solution(self.current_solution.id)
        except Exception as e:
            logger.error(f"Lỗi cập nhật visualization: {str(e)}")

    def _on_apply_clicked(self):
        """Xử lý khi nhấn nút Apply để chọn kế hoạch."""
        if not self.current_solution:
            QMessageBox.warning(
                self, "Thông báo", "Vui lòng chọn một giải pháp trước khi áp dụng."
            )
            return

        if self.pareto_navigator:
            # Ưu tiên sử dụng ParetoNavigator nếu có
            plan = self.pareto_navigator.create_plan_from_current_solution()
            if plan:
                self.planSelected.emit(plan)
                logger.info(
                    f"Đã tạo kế hoạch từ giải pháp Pareto: {self.current_solution.id if hasattr(self.current_solution, 'id') else 'unknown'}"
                )
                return

        # Fallback to old method if needed
        if hasattr(self, "mco_engine") and self.mco_engine:
            try:
                self.mco_engine.apply_solution(self.current_solution)
                plan = self.mco_engine.get_plan()
                if plan:
                    self.planSelected.emit(plan)
                    logger.info("Đã áp dụng giải pháp MCO thành công")
                else:
                    QMessageBox.warning(
                        self, "Lỗi", "Không thể tạo kế hoạch từ giải pháp đã chọn."
                    )
            except Exception as e:
                logger.error(f"Lỗi khi áp dụng giải pháp MCO: {str(e)}")
                QMessageBox.critical(
                    self, "Lỗi", f"Không thể áp dụng giải pháp: {str(e)}"
                )
        else:
            QMessageBox.warning(
                self,
                "Thông báo",
                "Không có MCO engine hoặc ParetoNavigator được cấu hình.",
            )

    def set_pareto_navigator(self, navigator: ParetoNavigator):
        """Thiết lập ParetoNavigator mới."""
        self.pareto_navigator = navigator

        if self.pareto_navigator:
            # Lấy danh sách mục tiêu từ navigator
            if hasattr(self.pareto_navigator, "get_objectives"):
                objectives = self.pareto_navigator.get_objectives()
                self._create_objective_sliders_from_dict(objectives)
            elif hasattr(self.pareto_navigator, "pareto_surface") and hasattr(
                self.pareto_navigator.pareto_surface, "objectives"
            ):
                objectives = self.pareto_navigator.pareto_surface.objectives
                self._create_objective_sliders_from_dict(objectives)

            # Cập nhật giao diện
            self._update_visualization()
            self.navigate_tab.setEnabled(True)
            self.apply_btn.setEnabled(True)

            logger.info("ParetoNavigator đã được thiết lập thành công")

    def _create_objective_sliders_from_dict(self, objectives):
        """Tạo slider từ danh sách mục tiêu."""
        self._clear_objective_sliders()

        for name, obj_info in objectives.items():
            # Xác định min/max từ obj_info (có thể là dict hoặc object)
            if isinstance(obj_info, dict):
                min_val = obj_info.get("min_value", 0.0)
                max_val = obj_info.get("max_value", 1.0)
                default_weight = obj_info.get("default_weight", 0.0)
            else:  # Đây là object
                min_val = getattr(obj_info, "min_value", 0.0)
                max_val = getattr(obj_info, "max_value", 1.0)
                default_weight = getattr(obj_info, "default_weight", 0.0)

            # Tạo slider mới
            slider = ObjectiveSlider(name, 0.0, 1.0, self)
            slider.valueChanged.connect(self._on_objective_slider_changed)
            slider.set_value(default_weight)
            self.objective_sliders[name] = slider
            self.objective_layout.addWidget(slider)

            # Cập nhật trọng số
            self.solution_weights[name] = default_weight
