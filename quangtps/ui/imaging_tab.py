...
from quangtps.ui.threshold_contour_tool import (
    ThresholdContourTool,
    AutoThresholdContourTool,
)
...
class ImagingTab(QWidget):
    ...
    def _connect_signals(self):
        ...
        if hasattr(self.contour_control, "tool_changed"):
            self.contour_control.tool_changed.connect(
                self.contour_tool_manager.set_active_tool
            )
        ...
        if hasattr(self.image_viewer, "sagittal_view"):
            self.image_viewer.sagittal_view.wheelEvent = self._on_wheel_event_sagittal

        if hasattr(self.image_viewer, "coronal_view"):
        ...
        for tool_name in [
            ...
            if tool and hasattr(tool, "contour_created"):
                tool.contour_created.connect(
                    self.contour_control.add_contour_from_points
                )
            if tool and hasattr(tool, "contour_updated"):
                tool.contour_updated.connect(
                    self.contour_control.add_contour_from_points
                )
    ...
    def _on_load_dicom(self):
        ...
        try:
            ...
            if not series_list:
                QMessageBox.warning(
                    self, "Lỗi", "Không tìm thấy dữ liệu DICOM trong thư mục đã chọn."
                )
                return
    ...
    def _display_current_series(self):
        ...
        try:
            ...
            if (
                not hasattr(self.current_series, "image")
                or self.current_series.image is None
            ):
                logger.info("Series không có đối tượng image, thử tải dữ liệu hình ảnh")

                # Nếu image không có sẵn, thử tải dữ liệu hình ảnh
                if (
                    hasattr(self.current_series, "image_data")
                    and self.current_series.image_data is not None
                ):
                    if not isinstance(self.current_series.image_data, np.ndarray):
                        logger.error(
                            f"Dữ liệu hình ảnh không phải là mảng NumPy: {type(self.current_series.image_data)}"
                        )
                        QMessageBox.warning(
                            self, "Lỗi tải", "Dữ liệu hình ảnh không hợp lệ"
                        )
                        return
                    ...
                    try:
                        ...
                        self.current_series.image.data = (
                            self.current_series.image_data.copy()
                        )  # Tạo bản sao để tránh lỗi tham chiếu

                        # Đảm bảo modality hợp lệ
                        if (
                            hasattr(self.current_series, "modality")
                            and self.current_series.modality
                        ):
                            self.current_series.image.modality = (
                                self.current_series.modality
                            )
                        else:
                            self.current_series.image.modality = (
                                "OT"  # Unknown modality
                        ...
                        if (
                            hasattr(self.current_series, "pixel_spacing")
                            and self.current_series.pixel_spacing
                        ):
                            try:
                                spacing_x, spacing_y = (
                                    self.current_series.pixel_spacing[0:2]
                                )
                                self.current_series.image.metadata[
                                    "pixel_spacing_x"
                                ] = float(spacing_x)
                                ...
                                logger.warning(
                                    f"Không thể thiết lập pixel_spacing: {str(e)}"
                                )

                        # Thiết lập slice thickness nếu có
                        if (
                            hasattr(self.current_series, "slice_thickness")
                            and self.current_series.slice_thickness
                        ):
                            try:
                                self.current_series.image.metadata[
                                    "slice_thickness"
                                ] = float(self.current_series.slice_thickness)
                            except (ValueError, TypeError) as e:
                                logger.warning(
                                    f"Không thể thiết lập slice_thickness: {str(e)}"
                                )
                        ...
                        QMessageBox.warning(
                            self, "Lỗi tải", f"Không thể tải module hình ảnh: {str(e)}"
                        )
                        return
                    except Exception as e:
                        logger.error(f"Lỗi khi tạo đối tượng Image: {str(e)}")
                        QMessageBox.warning(
                            self, "Lỗi tải", f"Lỗi khi xử lý dữ liệu hình ảnh: {str(e)}"
                        )
                        return
                else:
                    ...
                    try:
                        if (
                            not hasattr(self.current_series, "files")
                            or not self.current_series.files
                        ):
                            logger.error("Series không có danh sách files DICOM")
                            QMessageBox.warning(
                                self, "Lỗi tải", "Series không có danh sách files DICOM"
                            )
                            return

                        # Hiển thị thông báo tiến trình
                        progress_dialog = QProgressDialog(
                            "Đang tải dữ liệu hình ảnh...", "Hủy", 0, 100, self
                        )
                        ...
                        if not success:
                            ...
                            QMessageBox.warning(
                                self, "Lỗi tải", "Không thể tải dữ liệu hình ảnh DICOM"
                            )
                            return
                    except Exception as e:
                        logger.exception(
                            f"Lỗi trong quá trình tải dữ liệu hình ảnh: {str(e)}"
                        )
                        QMessageBox.warning(
                            self,
                            "Lỗi tải",
                ...
                if (
                    ...
                    logger.error(
                        "Không thể hiển thị series: không có dữ liệu hình ảnh sau khi tải"
                    )
                    QMessageBox.warning(
                        self,
                        "Lỗi hiển thị",
            ...
            if not hasattr(self.current_series.image, "data"):
                ...
                QMessageBox.warning(
                    self, "Lỗi hiển thị", "Đối tượng hình ảnh không có dữ liệu"
                )
                return
            ...
            if self.current_series.image.data is None:
                ...
                QMessageBox.warning(
                    self, "Lỗi hiển thị", "Dữ liệu hình ảnh không tồn tại"
                )
                return

            if not isinstance(self.current_series.image.data, np.ndarray):
                logger.error(
                    f"Dữ liệu hình ảnh không phải là mảng NumPy: {type(self.current_series.image.data)}"
                )
                QMessageBox.warning(
                    self,
                    "Lỗi hiển thị",
                    "Dữ liệu hình ảnh không hợp lệ (không phải NumPy array)",
                )
                return
            ...
            if self.current_series.image.data.size == 0:
                ...
                QMessageBox.warning(
                    self, "Lỗi hiển thị", "Đối tượng hình ảnh có dữ liệu rỗng"
                )
                return
            ...
            try:
                logger.info(
                    f"Tải hình ảnh vào ImageViewer, kích thước: {self.current_series.image.data.shape}"
                )
                ...
                try:
                    ...
                    if (
                        self.parent()
                        and hasattr(self.parent(), "statusBar")
                        and callable(self.parent().statusBar)
                    ):
                ...
                logger.info(
                    f"Hiển thị thành công series {self.current_series.description if hasattr(self.current_series, 'description') else ''}"
                )
                ...
                QMessageBox.critical(
                    self,
                    "Lỗi hiển thị",
                    f"Lỗi thuộc tính khi hiển thị series DICOM: {str(e)}",
                )
                return
                ...
                QMessageBox.critical(
                    self,
                    "Lỗi hiển thị",
                    f"Lỗi giá trị khi hiển thị series DICOM: {str(e)}",
                )
                return
            except Exception as e:
                logger.exception(f"Lỗi khi hiển thị series DICOM: {str(e)}")
                QMessageBox.critical(
                    self, "Lỗi hiển thị", f"Lỗi khi hiển thị series DICOM: {str(e)}"
                )
                return
        except Exception as e:
            ...
            QMessageBox.critical(
                self,
                "Lỗi hiển thị",
                f"Lỗi không xác định khi hiển thị series DICOM: {str(e)}",
            )
            return
